...
22. Known remaining limitations (original gotcha #21 was fully resolved with no residual rule - dropped; numbering below matches FULL-REFERENCE.md):
    ...
    - Passing a `bool`/`f64`/`f32` into an `any` slot is a clean compile-time error (`E0110`) in BOTH shapes now: the DIRECT `let x: any = <bool>` (LEDGER item 24) and the CONTAINER shape -- an `[any]` array element, a `map<_, any>` value, or an `args: [any]` parameter, e.g. `fn log_event(args: [any])` called as `log_event([true])` (LEDGER item 40). Neither type's native representation (`i1`/`double`) is i64-shaped the way `any`'s erased slot needs, so the value was being reinterpreted rather than converted: before the fix that one program printed THREE different values for `true` -- source says `true`, JIT printed `1`, AOT printed `-1` -- and the AOT path built clean, ran clean and exited 0. Rejecting is deliberate; a general fix needs the tagged-value ABI change that item 6 rules out.
    - STILL LIVE (LEDGER item 40b), and the reason the check above stops at `bool`/`f64`: a `str` routed through `[any]` renders a raw POINTER (`log_event(["hello"])` prints something like `140698792906752`). A `str` is already i64-shaped, so unlike `bool`/`f64` the handle survives intact and only the RENDERING is wrong -- a different mechanism, deliberately not blocked because `[any]` still appears in live stdlib signatures (`std::test::run_tests`, `std::result::to_array`, `std::iter::count`). `i64` through `[any]` is genuinely fine. Prefer a concrete element type (`[str]`) -- `std::fmt::format` was already migrated `[any]` -> `[str]` for exactly this reason.
