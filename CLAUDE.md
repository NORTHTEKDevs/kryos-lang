...
22. Known remaining limitations (original gotcha #21 was fully resolved with no residual rule — dropped; numbering below matches FULL-REFERENCE.md):
    ...
    - A DIRECT `let x: any = <bool>` / `let x: any = <f64>` is now a clean compile-time error (`E0110`, LEDGER item 24) instead of the old AOT-crash-plus-JIT-silent-misrender split -- neither type's native representation (`i1`/`double`) is i64-shaped the way `any`'s erased slot needs. This does NOT cover the same value routed through an `[any]` ARRAY element or an `args: [any]` parameter (e.g. `fn log_event(args: [any])` called with a bool) -- that shape still silently misrenders on BOTH backends with three disagreeing values for `true` (source says `true`, JIT prints `1`, AOT prints `-1`), unfixed, tracked as LEDGER item 40.
