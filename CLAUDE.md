...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Calling a struct-returning function through a fn VALUE returns garbage on `build --release`.** `let f = mk  let w = f()` where `mk() -> W` hands back a struct: reading `w.body` yields a raw pointer reinterpreted as data (a length in the trillions), while a DIRECT `mk()` is correct and `kryos run` is correct on both forms. The indirect-call path expects the callee to BOX an aggregate return and unboxes it with inttoptr+load, but a bare function reference stores the raw function pointer, so it unboxes something that was never boxed. That is why a ROUTER that stores handlers in a table and returns a `Response` struct misbehaves on the release build. Workaround: call the function directly, or have the handler return a `str`/handle rather than a struct. Fixing it needs an adapter thunk for a function reference whose return type is an aggregate.
