...
17. **`impl` methods on generic structs work, including parametric `impl<T>`.** All three forms parse and run on both backends: `impl Wrap { fn get(self: Wrap<i64>) -> i64 {..} }`, the concrete `impl Wrap<i64> { .. }`, AND the parametric `impl<T> Wrap<T> { fn get(self: Wrap<T>) -> T {..} }` (the impl's type params are scoped in the checker and erased to i64 slots in MIR). As with field access on generic structs, payload slots are i64-sized, so a method on a non-`i64` instantiation that returns the bare type parameter directly (e.g. `Box<str>.get()` typed as `str`) sees the raw i64 slot — annotate the method for the concrete instantiation you use (the common case). Multi-parameter impls (`impl<A, B> Pair<A, B>`) work too.
18. **Integer division/modulo, bounds, AND out-of-range float→int casts are all checked/defined.** Integer `a / 0` / `a % 0` panic ("integer division by zero"), array/string out-of-bounds panic, and signed division truncates toward zero (`-7 / 2 == -3`) consistently on both backends. Casting an out-of-range `f64` to an integer now **saturates** on BOTH backends (`1.0e300 as i64` → `i64::MAX`, `-1.0e300 as i64` → `i64::MIN`, `NaN as i64` → `0`) — no undefined behavior, no manual range guard needed. (This was previously UB on AOT; fixed.)
