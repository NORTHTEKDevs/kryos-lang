...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Known remaining memory leaks (unbounded growth, never a wrong value).** Most of the ARC model is now balanced -- heap arguments, call results, returned temporaries, closure/dyn results, slot overwrites (`m[k]=v`, `a[i]=v`, `s.f=v`), element reads and pushed values are all flat under a 4x-scale soak. Four shapes still grow linearly and matter only for long-running or high-call-count code:
      1. **A stdlib/user function that builds an array by `push` inside a nested block and returns it** -- `std::string::split` in a loop leaks ~870MB per 1M calls. `push(arr, v)` retains its RECEIVER (arg 0) as well as its value; that retain is skipped only when the result is assigned straight back (`a = push(a, v)`), so a push whose result lands in a different temp -- which is what a push inside an `if`/`while` body compiles to -- leaves the array's ref_count above 1 forever, and the array's own drop then skips freeing the buffer AND every element. Removing that receiver retain is NOT safe as-is: it breaks `Stack.push` on Cranelift (conf_stdlib_wave12/14), so the receiver retain is load-bearing somewhere else and the two paths need reconciling.
      2. **A struct literal holding heap fields, bound to a local in a loop** -- ~240MB per 1M. The drop helper is generated and frees every field correctly, and the drop is emitted inline, so the imbalance is upstream of the free.
      3. **A heap local inside a `try` block** -- ~69MB per 1M.
      4. **`string_builder()` per iteration** -- ~18MB per 1M.
      Workaround for all four: hoist the allocation out of the hot loop, or reuse one buffer. `tests/mem_plateau_check.sh` guards the shapes that ARE fixed; these four are measured with a PowerShell peak-RSS soak at two scales (a leak grows 4x with N, a fixed shape stays flat).
