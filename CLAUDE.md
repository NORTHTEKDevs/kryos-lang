...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **A self-referential struct through an ARRAY (`struct Tree { kids: [Tree] }`) is correct on `build --release` but still corrupts the heap on `kryos run`.** Building a tree and letting it drop exited 127 on BOTH backends until the struct-literal array-field clone learned to add an owner for STRUCT elements (a header clone shares the element boxes, so the source array's drop freed what the new field still pointed at). The LLVM path is fixed; the Cranelift path shares element boxes somewhere else and still dies. Use `build --release` for recursive-through-array data, or model the recursion through `map<K, T>` or `Option<T>` -- both are correct on both backends and covered by conf_recursive_struct. A cycle of PLAIN struct fields (`struct A { b: B }  struct B { a: A }`) is a compile error now: it is a value of infinite size and used to make the compiler itself stack-overflow with no diagnostic.
