...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Passing a heap value (`str`/`[T]`/`map`) to a USER-DEFINED function leaks it — roughly one buffer per call, on BOTH backends.** `fn consume(s: str) -> i64 { return len(s) }` called in a loop grows memory linearly (measured: 1M calls of a one-allocation loop peaked at 64MB on `build --release` and 52MB on `kryos run`, versus a flat ~4MB when the same string is passed to a BUILTIN like `len`). Runtime counters show zero free calls for the argument (`str_freeCALL=0`). ROOT: the caller marks a heap argument *consumed* on the theory that the callee takes ownership, but the callee records params as caller-owned and never drops them, so neither side frees. The two backends additionally disagree on the model — Cranelift retains every heap param in the callee prologue (a retain nothing releases), LLVM emits no such retain — so a fix must reconcile both, not just add a drop. NOT a correctness bug: values are always right and nothing is freed early; it is purely unbounded growth, and it only matters for long-running or high-call-count programs (a short script is unaffected). No clean workaround today beyond keeping hot loops on builtins/inlined code; a fix is the top open memory item.
