...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Memory: one known leak remains, and it is small and bounded.** The ARC model is otherwise balanced -- heap arguments, call results, returned temporaries, closure/dyn results, slot overwrites (`m[k]=v`, `a[i]=v`, `s.f=v`), element reads, pushed values, struct literals with heap fields, `try` bodies, `if`/`while` condition temporaries and array-building stdlib functions are all FLAT under a 4x-scale soak (verified 1M vs 4M iterations; a leak grows 4x, a fixed shape stays ~4MB). The exception: **each `string_builder()` you CREATE leaks ~50 bytes** (a raw `alloc(8)` cell plus its buffer). Appending does not leak -- one builder reused across 1M appends is flat -- so the cost is per builder, not per byte. The cell exists on purpose: `self` is a value copy, so `build()` needs a shared cell to null out, which is what makes a second `build()` return `""` instead of double-freeing (`87851a6`). Removing it needs struct destructors, which the language does not have. Workaround: hoist the builder out of a hot loop and reuse it.
