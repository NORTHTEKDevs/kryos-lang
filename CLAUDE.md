...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Memory: one known leak remains -- calling a METHOD on a struct leaks the receiver (~79MB per 1M calls).** Everything else is balanced: heap arguments, call results, returned temporaries, closure/dyn results, slot overwrites (`m[k]=v`, `a[i]=v`, `s.f=v`), element reads, pushed values, struct literals with heap fields, `try` bodies, `if`/`while` condition temporaries and array-building stdlib functions are all FLAT under a 4x-scale soak (1M vs 4M iterations; a leak grows 4x, a fixed shape stays ~4.4MB). `v.method()` marks the receiver MOVED into the callee while the callee treats params as caller-owned and never drops them, so nobody frees it; reading fields directly (`v.a`) is flat. This is also why each `string_builder()` costs ~50 bytes -- `sb.append(..)` is a method call. **WHY IT IS NOT SIMPLY FIXED, settled by experiment:** a struct box has NO REFCOUNT. `kryos_alloc`/`kryos_free` are plain malloc/free with a size header, unlike `str`/`[T]`/`map`, which carry a real `ref_count`. So the sharing sites CANNOT retain a struct, and the leak is exactly what makes sharing safe today. Borrowing struct receivers has been tried and reverted four times, each attempt surfacing another site that assumes nobody frees: a spawned thread reads `dropped` out of a freed mutex (conf_spinlock_mutex), and `x = x.method()` blows up when the returned struct shares the receiver's fields -- `Agent { tools: push(self.tools, tool) }` hands back the SAME array, and an array of STRUCTS is duped shallowly (element kinds cover str/array/map only), so releasing the old receiver frees elements the new one still points at (segfault; conf_stdlib_wave14 exits 127). Closing this properly means giving struct values a refcount header -- an ABI change across both backends, every struct literal and field access, spawn capture, containers and the self-host compiler -- not a local fix. Workaround for a hot loop: read fields directly, or reuse one instance instead of constructing per iteration.
