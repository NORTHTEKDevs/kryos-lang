...
23. **Struct param/copy semantics across backends (PARTIALLY RESOLVED).** **All-scalar `@copy` struct params** (no str/array/map/struct/fn fields) are now copied at function entry on the JIT, so a field mutation inside the callee no longer aliases the caller's value — both backends agree on pass-by-value for plain-data structs (regression test: `tests/smoke/test_copy_param_value_semantics.kry`). Residual divergences:
    ...
    - **Non-`@copy` structs:** JIT aliases, AOT copies. Mostly unobservable, BUT NOT always benign: reassigning a non-`@copy` struct from a `match`-bound enum payload that shares the struct's OWN heap field, inside a loop, corrupts/crashes -- e.g. a state-machine loop doing `match tok { Op => { let r: Result<Calc,str> = Ok(Calc{stack: c.stack})  match r { Ok(nc) => { c = nc } .. } } .. }` where `Calc` has a heap field: AOT SEGFAULTS (exit 139) on one element order, JIT drops output/crashes on the reverse order (a refcount imbalance in the AOT deep-copy-on-reassign path interacting with `release_if_ne`; needs a heap field, the loop, and the nested-match-payload reassign together -- remove any one and it is clean). This is a common token-processing/state-machine shape. **Workaround: annotate the struct `@copy`** (verified fixes both backends + both orders), or avoid reassigning the loop var from a nested-match payload that aliases its own field. Deep ARC fix backlogged (delicate path, high regression risk). The self-host compiler relies on the JIT aliasing internally (lower.kry ctx fns).
