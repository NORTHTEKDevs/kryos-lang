...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Memory: one known leak remains -- calling a METHOD on a struct leaks the receiver.** Everything else is balanced: heap arguments, call results, returned temporaries, closure/dyn results, slot overwrites (`m[k]=v`, `a[i]=v`, `s.f=v`), element reads, pushed values, struct literals with heap fields, `try` bodies, `if`/`while` condition temporaries and array-building stdlib functions are all FLAT under a 4x-scale soak (1M vs 4M iterations; a leak grows 4x, a fixed shape stays ~4.4MB). The exception: `v.method()` marks the receiver MOVED into the callee, but the callee treats params as caller-owned and never drops them, so the struct box and its heap fields are never freed -- ~79MB per 1M method calls. Reading fields directly (`v.a`) is flat, and this is also why each `string_builder()` costs ~50 bytes: `sb.append(..)` is a method call. Simply borrowing struct receivers (the fix that worked for str/array/map) is NOT safe and has been tried and reverted three times: a struct handle is shared by `spawn` snapshots and container reads, so the caller's drop frees it under another live holder -- conf_spinlock_mutex reads `dropped` out of freed memory and conf_stdlib_wave14 exits 127. A correct fix needs struct handles refcounted at the sharing sites (spawn capture, container read) first. Workaround for a hot loop: read fields directly, or reuse one instance instead of constructing per iteration.
