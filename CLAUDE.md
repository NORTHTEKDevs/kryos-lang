...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Memory: one known leak remains -- calling a METHOD on a struct leaks the receiver (~79MB per 1M calls).** Everything else is balanced: heap arguments, call results, returned temporaries, closure/dyn results, slot overwrites (`m[k]=v`, `a[i]=v`, `s.f=v`), element reads, pushed values, struct literals with heap fields, `try` bodies, `if`/`while` condition temporaries and array-building stdlib functions are all FLAT under a 4x-scale soak (1M vs 4M iterations; a leak grows 4x, a fixed shape stays ~4.4MB). `v.method()` marks the receiver MOVED into the callee while the callee treats params as caller-owned and never drops them, so nobody frees it; reading fields directly (`v.a`) is flat. This is also why each `string_builder()` costs ~50 bytes -- `sb.append(..)` is a method call. **WHY IT IS NOT SIMPLY FIXED, settled by experiment:** the leak is what makes struct sharing safe today, and the obstacle is the struct REPRESENTATION, not a missing bit. A refcount slot is actually free: `kryos_calloc` (which boxes structs) already reserves a 16-byte header and uses only the first 8 for its class tag, and the header is invisible to codegen, so an owner count in the second word costs no layout change and is backward compatible (0 extra owners = today's behaviour). That half works, and it fixes one of the two sharing sites: teaching `kryos_array_dup` a STRUCT element kind (retain each element rather than sharing it blindly) turned conf_stdlib_wave14 from a teardown segfault into a pass -- `Agent { tools: push(self.tools, tool) }` hands back the SAME array handle, and an array of structs was duped shallowly. The blocker is the OTHER site: a struct VALUE is not uniformly a heap pointer. Small structs are SSA aggregates passed `byval`/`sret`, so retaining a returned `self` emits `call kryos_struct_retain(%P %t1)` -- the aggregate itself, with no header to count. That is exactly the shape conf_spinlock_mutex needs (`fn lock(self: SpinLock) -> SpinLock { .. return self }`, where `let l = lock.lock()` becomes a second owner). A fifth attempt went further and mapped the remaining work exactly: a returned `self` is a SHALLOW aggregate copy, so the fix there is retaining the struct's heap FIELDS at the return boundary (not the box), and an array of structs needs its own dup element-kind so the copy adds an owner. Both work -- with them, conf_stdlib_wave14's assertions pass and the std::agent `ag = ag.add_tool(..)` repro stops segfaulting. What still fails is conf_spinlock_mutex (segfault) and wave14's teardown (rc=127), because a struct's heap fields are shared through MORE paths than the return boundary -- parameters, containers, spawn capture and reassignment all hand out shallow copies, and today's safety rests on nobody freeing any of them. Finishing this means converting every one of those sites together (or boxing struct values uniformly), which is a representation and ABI change with a real performance cost -- not an incremental patch. Borrowing struct receivers has been tried and reverted four times; each attempt surfaced another site that assumes nobody frees. Workaround for a hot loop: read fields directly, or reuse one instance instead of constructing per iteration.
