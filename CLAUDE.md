...
18. **Integer division/modulo, bounds, AND out-of-range float→int casts are all checked/defined.** Integer `a / 0` / `a % 0` panic ("integer division by zero"), `i64::MIN / -1` and `% -1` panic ("integer division overflow" — the quotient is unrepresentable), array/string out-of-bounds panic, and signed division truncates toward zero (`-7 / 2 == -3`) consistently on both backends. Casting an out-of-range `f64` to an integer now **saturates** on BOTH backends (`1.0e300 as i64` → `i64::MAX`, `-1.0e300 as i64` → `i64::MIN`, `NaN as i64` → `0`) — no undefined behavior, no manual range guard needed. (This was previously UB on AOT; fixed.) **One narrow known edge:** `parse_float("-0.0")` yields `+0.0` on the AOT backend (the sign of a parsed *zero* is dropped), while `kryos run`/JIT preserves it -- so `1.0 / parse_float("-0.0")` is `+inf` on AOT vs `-inf` on JIT. Only negative ZERO is affected: non-zero negatives (`parse_float("-5.0")`) and the literal `-0.0` are correct on both backends. Almost no program depends on the sign of a parsed zero; if you do, construct it as `-1.0 * 0.0` instead. (Backlogged.) **Compiler note (libm-name collision):** on the AOT backend, LLVM's constant-folding recognizes a function whose name + `(f64)->f64` signature matches a libm routine (`atan`, `sin`, `pow`, ...) and folds a CONSTANT-argument call to libm's exact value, ignoring the Kryos body. For the std::math functions this is harmless (they match libm to ~1 ULP), but if you define your OWN function named like a libm routine with DIFFERENT behavior, a constant-argument call may return libm's value on AOT while `kryos run`/JIT runs your body -- avoid shadowing libm names, or pass a runtime (non-constant) argument.
...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    - **`std::collections::List<T>`/`Set<T>` with a NON-i64 element type fails to build on AOT (use a built-in `[T]` array instead).** The no-argument generic static constructor `List.new()` cannot infer `T` from the call, and the `let ls: List<str> = ...` annotation is not propagated into its monomorphization, so `new()` defaults to `List___i64` while the chained `.push("a")` wants `List___str` -- a type mismatch that segfaults/prints raw pointers on JIT and hard-fails LLVM codegen on AOT (`'%_N' defined with type '%List___i64' but expected '%List___str'`). `List<i64>` works fine on both backends. Workaround: for `str`/`f64`/struct elements use the built-in `[T]` array (`let xs: [str] = []; xs = push(xs, "a")`), which is fully supported, or a `map` for keyed values. (Bidirectional generic inference into no-arg static-method monomorphization is backlogged.)
    ...
    - **A generic function that RETURNS a closure fails the AOT build when instantiated at a pointer-backed type (`str`/struct).** `fn make_appender<T>(suffix: T) -> fn(T) -> T { return |x| x + suffix }` then `make_appender("!")` runs correctly on `kryos run`/JIT (`"hi!"`) but hard-fails `build --release` with `'%_N' defined with type 'ptr' but expected 'i64'`: the returned closure's `T` is not monomorphized to `str` through the closure env, the `ret`, AND the call site together, so the erased-`i64` return signature collides with the `ptr` string-concat value. `T=i64` works on both backends. This is a LOUD build failure (not a silent miscompile), so the safe move is to notice it. Workaround: return a concrete `fn(str) -> str` (drop the type param) or use `kryos run`. (Full monomorphization of a returned generic closure at a pointer type is backlogged -- same erasure family as the `List<T>`/`any` gaps.)
    - **`any` is type-erased to i64, so `to_string`/`std::fmt::format` mis-render non-i64 `any` values.** The `any` type carries NO runtime type tag (it lowers to a bare i64 slot). A `str`/`f64` stored in an `[any]` (e.g. an `[str]` passed to a `fn(args: [any])`, or built via `push(args, "x")`) reads back through `to_string` as its raw pointer/bit representation, not its logical value -- so `std::fmt::format("{{0}}", args)` only formats i64 args correctly; str/f64 args print garbage. This is inherent to the erasure (fixing it needs tagged `any` values). Workaround: build the final string with `+` and per-type `to_string`/`float_fixed` calls on the concretely-typed values instead of routing through `[any]`/`format`.
...
23. **Struct param/copy semantics across backends (PARTIALLY RESOLVED).** **All-scalar `@copy` struct params** (no str/array/map/struct/fn fields) are now copied at function entry on the JIT, so a field mutation inside the callee no longer aliases the caller's value — both backends agree on pass-by-value for plain-data structs (regression test: `tests/smoke/test_copy_param_value_semantics.kry`). Residual divergences:
    ...
    - **Non-`@copy` structs:** JIT aliases, AOT copies. Mostly unobservable, BUT there is a BROAD, HIGH-PRIORITY crash hazard (TOP known bug, needs a dedicated memsan fix -- NOT yet fixed): a non-`@copy` struct with a HEAP field (array/str), reassigned inside a LOOP from a NESTED-`match`-bound enum payload (`match r { Ok(nc) => { c = nc } .. }`), where the loop dispatches via a MULTI-ARM enum `match` and another arm populates the field, CRASHES -- AOT SEGFAULTS (exit 139), JIT drops output / crashes (exit 127), order-dependent. Root: a refcount/ownership imbalance across the match-payload `kryos_array_dup` + the AOT deep-copy-on-assign (`kryos_array_clone`) + the Result/enum `drop`. **Aliasing the old field is NOT required** (a fresh `Ok(S{data: []})` payload still crashes), and **`@copy` does NOT reliably fix it** -- it fixes the AOT segfault but the JIT still crashes (exit 127) on the reverse element order and on a realistic RPN-calculator shape. This IS the standard "validate-then-commit via `Result`" idiom for fallible state transitions: ordinary, non-adversarial interpreter/parser/RPN-calculator loops hit it on first try. NECESSARY+SUFFICIENT shape (mapped over 31 variations, Pass 23): heap-field struct + real loop (>=2 iters across the back-edge; hand-unrolling is clean) + multi-arm enum `match`/`if let` dispatch (single-arm or `if/else` is clean) + one arm reassigning the loop var through a NESTED match/if-let payload binding (a DIRECT `c = nc`/`c = f(c)`/`c = T{..}` in that arm is clean). PARTIAL workaround: restructure so the fallible arm reassigns the loop var DIRECTLY (unwrap the `Result` into a local, then `c = local`, NOT `match r { Ok(nc) => c = nc }`), or avoid a heap field in the loop-carried struct. Deep fix (dedicated session, memory sanitizer) is the #1 backlog item; the fix must not regress self-host, which relies on the JIT non-`@copy` aliasing internally (lower.kry ctx fns). Full repro set + variation table: scratchpad `f1probe/`.
