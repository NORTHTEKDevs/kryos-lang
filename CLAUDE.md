...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **Memory: a TCP round trip leaks ~50 bytes (open).** Repro + full rule-out list: `tests/mem/tcp_roundtrip_leak.kry`. A keep-alive server grows linearly and without bound: 40k round trips +1.6MB, 120k +5.8MB, 360k +18.7MB. It scales with ROUND TRIPS, not connections (60k round trips over 60k connections vs over 60 connections leak the same), and is independent of payload size. Already ruled out: the Kryos-side string IS freed (verified in the emitted IR), it is not the receive buffer (size-independent), not the string header pool (an unbounded recycle list, so headers are reused), not `io_offload` (allocates nothing), and not socket duplication (replacing `try_clone` in send and recv with a non-owning borrow of the raw handle left it unchanged, so that change was reverted rather than kept as unverified unsafe code). Careful with controls here: a server that accepts and sends WITHOUT reading looks flat but is NOT equivalent work -- closing without reading makes Windows send RST and 3965 of 4000 requests actually fail.
