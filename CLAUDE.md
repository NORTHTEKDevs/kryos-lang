...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    - **Untyped array of aggregates:** `let mut a = []; a = push(a, Struct{..}); a[i].field` fails to type-check (element type stays unresolved). **Annotate: `let mut a: [Struct] = []`** (works on both backends). (Fix needs push generics + MIR element inference + AOT aggregate-element codegen.)
    - **Narrow-int literal assignment:** `let x: u8 = 200` errors (int literals are i64). **Use `let x: u8 = 200 as u8`.** (Fix needs int-literal polymorphism.)
    - **Nested struct field mutation:** `o.a.v = 99` (mutating a field of a struct-typed field) works on JIT but the MIR copies the inner struct, so AOT mis-mutates/fails. **Rebuild instead: `o.a = Inner { v: 99 }`** or use `kryos run`.
    - **`Option`/`Result` with a multi-field (>8-byte) struct payload** (e.g. `Option<User>` where `User` has 2+ fields): JIT works; AOT codegen has a box/unbox edge. **Use a single-field newtype, an i64/handle payload, or `kryos run`.**
