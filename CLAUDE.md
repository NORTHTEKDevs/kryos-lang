...
13. **`Result`/`Option` payloads carry their real VALUE when matched, but the binding is TYPED `i64` (the uniform-slot ABI erases enum payloads).** `match r { Result.Err(e) => ... }` binds `e` to the right str pointer, and string ops reinterpret it correctly — `"prefix " + e`, `to_string(e)` (identity on str), `len(e)`, `e == "x"` all work. BUT a *bare* `println(e)` dispatches on the i64 binding type and prints the str pointer as a **number** (garbage), since payload bindings erase to i64. **Workaround: never bare-`println` a `Result`/`Option` payload directly — use `println("" + e)` or `println(to_string(e))`.** (Known limitation, both backends; rooted in enum-payload i64-slot erasure. Always annotate `Result<T,E>`/`Option<T>` on signatures regardless.)
