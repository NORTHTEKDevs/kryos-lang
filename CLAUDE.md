...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **`dyn Trait` inside ANY container is NOT supported (avoid — it CRASHES).** A single `dyn Trait` value, parameter (including trait-default-method and `Type::method(..)` static params), field, return, or let-binding works and dispatches correctly on both backends. But a trait object stored INSIDE a container is unimplemented across the board: `[dyn Shape]` arrays, `Option<dyn Shape>` / `Result<dyn Shape, E>` payloads, `(i64, dyn Shape)` tuple elements, and `map<K, dyn Shape>` values all SEGFAULT (or hang) on both backends — the container element store assumes a scalar slot, not a vtable fat pointer. A heterogeneous `[dyn Shape]` literal is additionally rejected at type-check. Workaround: use an enum with a variant per concrete type and `match` (fully supported), or keep separate typed arrays. (Fat-pointer container storage is backlogged.)
