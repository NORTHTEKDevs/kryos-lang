...
22. Known remaining limitations (original gotcha #21 was fully resolved with no residual rule - dropped; numbering below matches FULL-REFERENCE.md):
    ...
    - Raw-memory builtins (`alloc`, `free_bytes`, `ptr_read_i64`, `ptr_write_i64`, `ptr_byte_at`, `ptr_set_byte`, `str_to_ptr`) are **capability-gated on `ffi`** (`E0505`), though still not gated by `unsafe {}`. This entry previously claimed they need NO capability and were an ungated unsafe surface; re-measured 2026-08-14, that is FALSE and was understating security. Both pointer SOURCES (`alloc`, `str_to_ptr`) require `ffi`, so the raw-memory surface is closed at its entry points -- you cannot obtain a pointer without declaring `ffi`. Verified by `tests/authority_surface_gate.sh`, which checks every capability-classified builtin in both directions.
    ...
    - `comptime {}` is **expression-only**: write `let x = comptime { 6 * 7 }`. It is NOT a compile-time evaluator (real compile-time eval is deferred past 1.0), and MIR lowering keeps only the block's VALUE. A `comptime` block in STATEMENT position, or a side-effecting statement inside one, is now a clean `E0110` compile error rather than silently doing nothing. This entry previously claimed such blocks "run at RUNTIME like an ordinary block ... don't rely on them for side-effect suppression"; measured 2026-08-14 that was FALSE in a dangerous way -- `println` inside a comptime block never reached MIR at all and vanished, while an assignment beside it survived as `_0 = const 99_i64` and took effect. Both shapes are rejected now, so the silent split is gone (LEDGER item 42, pinned by `tests/diagnostics_gate.sh`).
    ...
    - `any` is type-erased to i64 - `to_string`/`std::fmt::format` on a `str`/`f64` value routed through a DIRECT `any` slot (`let x: any = "hi"`, LEDGER item 24's remaining territory for `str`/`f64` -- STILL LIVE) prints the raw bit pattern, not the value. Build such strings manually with `+` and per-type `to_string`/`float_fixed` instead. The element-typed `std::iter` HOFs (`map`, `filter`, `find`, `sum`, `enumerate`, `zip`, `unzip`, `chunks`, `windows`, `flat_map`, ...) ARE generic and preserve `str`/`f64` correctly.
    - Passing a `bool`/`f64`/`f32`/`str` into an `any` CONTAINER slot (an `[any]` array element, a `map<_, any>` value, or an `args: [any]` parameter, e.g. `fn log_event(args: [any])` called as `log_event([true])` / `log_event(["hi"])`) is a clean compile-time error (`E0110`, LEDGER items 40 + 40b) -- the DIRECT non-container shape (`let x: any = <bool>`, item 24) is also rejected for `bool`/`f64`/`f32` but, per the line above, still silently mis-renders `str`. `bool`/`f64`/`f32` are REINTERPRETED at a container `any` boundary (native representation `i1`/`double` is not i64-shaped, so the value is genuinely corrupted -- before the fix one program printed THREE different values for `true`: source says `true`, JIT printed `1`, AOT printed `-1`, with a clean AOT build and exit 0). `str` is a DIFFERENT mechanism -- a `str` handle is already i64-shaped (a pointer) so it survives a container `any` boundary intact and only RENDERS wrong on read-back (`log_event(["hello"])` printed a raw pointer like `140698792906752`, not `hello`) -- but both are rejected at the container boundary now. `i64` through `[any]` is genuinely fine (already the erased representation). Rejecting is deliberate; a general non-rejecting fix needs the tagged-value ABI change that item 6 rules out.
