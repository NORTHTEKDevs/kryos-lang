...
23. **Param-field mutation semantics differ between backends (pre-existing).** Mutating a field of a struct-typed parameter (`fn f(o: Outer) { o.a.v = 5 }`) compiles and runs on both backends, but on JIT the mutation writes through to the CALLER's struct (pass-by-reference aliasing), while on AOT it mutates the callee's byval copy (true pass-by-value). For non-Copy structs the ownership move rules make this unobservable; for all-Copy structs a caller that reads its struct after the call sees different values per backend. **Portable pattern: copy the param into a `let mut` local before mutating** (`let mut local = o`), or return the modified struct. The self-host compiler relies on the JIT aliasing internally, so the JIT behavior cannot change until those call sites are rewritten.
