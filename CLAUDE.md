...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **`std::json` stores every number as `f64`, so a JSON INTEGER above 2^53 loses precision.** `1234567890123456789` parses (and `to_int`s) back as `1234567890123456768`; `9007199254740993` becomes `9007199254740992`. Up to 2^53 round-trips exactly, as do floats. This is the JavaScript contract and the reason APIs send large IDs as strings -- do that here too. A proper fix needs an `Int(i64)` variant on `JsonValue`, which is a breaking change to that enum.
