...
20. **Closure params keep their real `str`/struct/array/**float** type across a HOF boundary.** An un-annotated closure passed to a higher-order function now carries its inferred param type into the closure body, so `fold(words, "", |acc, x| acc + x)` concatenates strings (not integer-adds their handles) and `fold(nums, 0.0, |acc, x| acc + x)` over `[f64]` sums as floats (the uniform i64 closure ABI bit-casts floats in/out via the env-thunk on both backends). **Limitation:** a HOF that *returns* a generic `[U]` array whose element is a non-i64-handle type — notably `[f64]` from `map([1.0, 2.0], |x| x * 2.0)` — reads its elements back as raw i64 slots unless you annotate the binding: `let m: [f64] = map(...)`. Scalar generic returns (`fold`/`reduce`/`scan` accumulators) don't need this; only generic `[U]` results with float elements do (same i64-slot-erasure class as gotcha #17).
