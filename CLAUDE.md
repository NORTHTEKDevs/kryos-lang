...
22. **Known remaining limitations (use `kryos run`/JIT or the workaround on AOT) — the shrinking bug tail:**
    ...
    - **RESOLVED: calling a struct-returning function through a fn VALUE.** `let f = mk  let w = f()` (and a handler TABLE, `let t: [fn() -> Resp] = [a, b]  t[0]().body`) now returns the real struct on `build --release`. Was: `w.body` read a raw pointer reinterpreted as data (a length in the trillions) -- the exact shape a ROUTER uses to store handlers and return a `Response`, so release-built routers misbehaved while `kryos run` was correct. Root cause: an aggregate return uses the `sret` ABI (`define void @mk(ptr sret(%W))`), but the generated env-thunk called it as `%t = call %W @mk()` and boxed the garbage register. The thunk now consults the same `func_sig_aggs` signature registry the definition side uses -- when the callee is sret it allocates the box FIRST and passes it as `ptr sret(%W)`, which is both correct and one copy cheaper. Gating on the registry (not on "the LLVM type looks like an aggregate") matters: an enum like `Result` is aggregate-SHAPED but returned directly, and guessing from the type string broke `Result.and_then` with a named fn. Regression: conf_struct_str_field.
