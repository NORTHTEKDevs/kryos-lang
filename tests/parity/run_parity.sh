...
# NOTE: there are intentionally NO tracked/allowed platform failures - every
# test is a hard gate on Cranelift JIT AND LLVM AOT across Windows, macOS, and
# Linux. (The Linux-glibc exception segfault that was briefly tracked here is
# fixed: LLVM now zero-inits enum/struct/ptr locals.)
declare -a ROWS=()
PASS_BOTH=0
FAIL_LLVM=0
FAIL_CL=0
