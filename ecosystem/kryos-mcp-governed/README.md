...
5. **`fetch` and `summarize` ship as stubs.** `fetch` declares `net` (its intended surface) but the MVP body returns a placeholder so local testing needs no outbound access. `summarize` declares `net` + `@budget(tokens=2000, calls=1)` and returns a placeholder; the real implementation reads the API key via `env_get` (which requires the `process` capability) and calls `std::llm`, so its production annotation is `@capabilities(net, process)` -- a correction to the original design sketch, which declared only `net`.
6. **Compiler gap found: `http2_get` does not link on the LLVM release backend.** It compiles and runs under `kryos run` (Cranelift) but `kryos build --release` fails with `use of undefined value '@http2_get'` (the LLVM call-site name mapping is missing). That is why `fetch` ships as a stub rather than a live `http2_get` call; net-gated builtins that DO link on LLVM are `tcp_*`, `tls_*`, `ws_*`.
