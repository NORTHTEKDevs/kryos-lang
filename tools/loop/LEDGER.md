...
### 53. DIAGNOSTIC GAP (found while closing item 50, 2026-08-29): a TWO-SEGMENT module path in expression position (`std::result::to_array(r)`) does not parse at all -- NOT FIXED, newly logged

`kryos check` reports `E0003: unexpected token '::', expected expression`
pointing at the second `::`. Only the ONE-segment alias spelling
(`result::to_array(r)`, which item 50 fixed) is accepted, even though the
`use std::result::{to_array}` import that makes the alias available is itself
spelled with the full two-segment path. This is an HONEST failure -- a clean
check-time diagnostic with a correct span, not a silent wrong answer or a
build failure -- so it is ranked well below items 51/52 and was deliberately
left out of item 50's scope rather than fixed opportunistically. What is
wrong is the message, not the rejection: E0003 is a generic parse error that
does not tell the user the one-segment spelling is the supported one.
Reproduce: `let a: [i64] = std::result::to_array(r)`. Measured live
2026-08-29 on the item-50 build.

### 51. LEAK -- item 49 RESIDUAL: struct-field assignment holding an Enum/Struct (`h.v = Val.ListV(..)`) still leaks on container-slot OVERWRITE; only the array-index/map-index shapes were fixed (2026-08-28) -- NOT FIXED, newly split off
