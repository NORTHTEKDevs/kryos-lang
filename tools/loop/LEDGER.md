...
### 49. LEAK -- item 45 RESIDUAL: the enum-array-payload leak still fires on container-slot OVERWRITE (`arr[i] = v` / map insert); only the fresh-push-and-drop shape was fixed (adversarial verification + independent re-measurement, 2026-08-28) -- NOT FIXED, item 45 REOPENED

Item 45 was moved to the CLOSED table on 2026-08-27 with the framing "FIXED
(not a NOT-DONE stretch outcome -- full fix landed)". The
`drop_unescaped_str_temps` `RValue::EnumVariant` arm added in
`kryos-mir/src/lower.rs` is real, correctly targeted, and genuinely closes the
shape its gate exercises -- but it closes ONE data-flow path into the gap. The
OVERWRITE path still leaks proportionally.

Measured 2026-08-28 by the orchestrator on the pushed post-fix binary
(`compiler/target/release/kryos.exe` built 00:24 after the fix commit
`eea72d57` at 23:58; fix presence PROVEN on the same binary by
`tests/mem_enum_array_push_gate.sh` -> **PASS, AOT 4MB / JIT 4MB at 5,000,000
iters**), using the same PowerShell `PeakWorkingSet64` polling technique as
that gate:
...
fn main() {
    // ... slots pre-allocated ONCE, then the slot is OVERWRITTEN every iteration
    let mut slots: [Val] = [Val.ListV([0, 0, 0])]
    while i < iters {
        slots[0] = Val.ListV([i, i + 1, i + 2])     // <-- overwrite, not push
        match slots[0] { Val.ListV(xs) => { acc = acc + len(xs) }, }
        i = i + 1
    }
}
```

| shape | 500k iters | 3M iters | verdict |
|---|---|---|---|
| push into freshly-scoped array (item 45's gate) | -- | 4MB @ 5M | FLAT, fixed |
| **overwrite an existing slot (this item)** | **66MB** | **371MB** | **LEAKS, ~124MB/M iters** |

Both shapes construct the identical `Val.ListV([..])` value on the identical
binary; only the destination differs. The wave's own adversarial verifier
independently isolated the same residual (its numbers: ~30-60MB/M iters
depending on container type, and it confirmed a plain NON-enum array value
overwritten the same way stays flat at 4.4MB -- so this is enum-and-array-
payload specific, not a general overwrite bug).

**STRONG CORROBORATION, already in this file:** item 44's own CLOSED entry
(WAVE 1 addendum) flagged exactly this, unprompted, and explicitly left it
unmeasured -- *"`release_if_ne_fn` also never covers Struct/Enum, so
overwriting an existing Struct/Enum-valued map/array slot leaks the old value
-- not measured or probed this session."* Two independent findings plus that
prior note all point at the same place. **Start there:** `release_if_ne_fn`'s
missing Struct/Enum coverage on the `arr[i] = v` / `m[k] = v` value-overwrite
release path, which is a DIFFERENT mechanism from the
`drop_unescaped_str_temps` temp-tracking pass item 45's fix touched -- that is
why the fix does not reach this shape.

**DO NOT** re-close item 45 on the strength of its existing gate:
`tests/mem_enum_array_push_gate.sh` only exercises the push-and-drop shape and
passes green while this leaks. Any fix here needs its own overwrite-shape leg
in that gate (or a sibling gate) before the item can close honestly.
