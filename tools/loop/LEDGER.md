...
### 50. BUILD FAILURE (not yet root-caused): a qualified-path call to a GENERIC stdlib function (`result::to_array(...)`) passes `kryos check` clean and then fails at LINK time with an unresolved-symbol error, instead of a clean diagnostic -- split off from item 48 when item 48 closed (2026-08-28) -- NOT FIXED, newly logged

Found incidentally while reproducing/fixing item 48 (destructuring bypass of
the unbindable-generic check), but a DIFFERENT bug class -- not a silent
wrong answer, a build failure, and unrelated to destructuring patterns. A
qualified-path call to a GENERIC stdlib function (`result::to_array(...)`)
passes `kryos check` clean and then fails at LINK time with an
unresolved-symbol error instead of a clean diagnostic. A same-shaped
qualified call to an ordinary NON-generic stdlib function links and runs
fine, so this is specific to qualified-path calls into monomorphized
generics. Not root-caused this session; logged so it is not lost. Not
reproduced fresh this session (carried forward from the prior finding as-is
-- re-confirm before acting on it).
