...
### 48. SILENT WRONG ANSWER -- item 40c RESIDUAL: the new unbindable-generic check only covers a BARE `let x = call(..)` binding; tuple-destructuring bypasses it entirely (adversarial verification + independent re-measurement, 2026-08-28) -- NOT FIXED, item 40c REOPENED

Item 40c was moved to the CLOSED table on 2026-08-27 by the `to-array-40c`
wave. Its fix is real but NARROWER than its CLOSED entry claims: the
`type_mentions_var` signature-shape check fires only on a single-identifier
`let` binding, and does not walk destructuring patterns. Measured live
2026-08-28 on the pushed HEAD binary (post-fix, `KRYOS_STDLIB_DIR` set):

```
use std::result::{to_array}
fn main() {
    let a = to_array(Ok("hi-there"))          // correctly REJECTED (E0110) -- the fix works
    println(a[0])
}

use std::result::{to_array}
fn main() {
    let (a, n) = (to_array(Ok("hi-there")), 5)   // check rc=0, NO diagnostic
    println(a[0])                                 // prints 140695407439872 (raw pointer)
}
```

`kryos check` exits 0 on the second shape and both backends print the raw
pointer -- the identical silent-wrong-answer class item 40c was opened for,
reached through a pattern the fix never walks. Independently reproduced
twice: once by the wave's own adversarial verifier, once by the orchestrator
re-running it fresh on the pushed binary.

**FIX SHAPE (from the verifier, not yet attempted):** extend the check to walk
into `Pattern::Tuple` (and by the same argument struct- and array-patterns)
destructuring targets wherever an RHS element is itself an unbindable-generic
`FnCall`, not just a bare `let name = call(..)`. The existing check is the
right mechanism; only its pattern coverage is incomplete.

**SEPARATE, LOWER PRIORITY, found in the same pass (not part of this item):**
a qualified-path call to a GENERIC stdlib function (`result::to_array(...)`)
passes `kryos check` clean and then fails at LINK time with an unresolved-symbol
error instead of a clean diagnostic. A same-shaped qualified call to an ordinary
NON-generic stdlib function links and runs fine, so this is specific to
qualified-path calls into monomorphized generics -- a build failure, not a
silent wrong answer, and a different bug class. Not root-caused; logged so it
is not lost.
