...
### 17. SUPPLY CHAIN: a dependency's explicit `git = "..."` / `github:org/repo@ver` source in `kryos.toml` is NEVER consulted by `kryos pkg install`/`update` — silently replaced by a pure by-name lookup against the single hardcoded official registry, or a flat failure, either way ignoring what the manifest says (RED TEAM round 3, toolchain-supply lens, found 2026-08-04) — **FIXED 2026-08-13, explicit-source wave. `install()`/`update()` now honor a non-empty `DepSpec::Remote.source` directly via a new `fetch_explicit_source`, never falling back to a by-name registry lookup for that dependency. This entry's repro (`tests/security/pkg_manifest_git_source_ignored.sh`) is kept below as the historical record and was rewritten in place as a regression pin asserting the fixed behavior (both directions still proven live). See CLOSED table for the fix and full proof-both-ways evidence.**
...
### 14. RESOURCE-DOS: `parse_statement`'s stray-`;` recovery recurses with NO nesting/depth guard — a flat run of semicolons stack-overflows the compiler on `kryos check` (RED TEAM round 2, resource-dos lens, found 2026-08-04) —  **FIXED 2026-08-14**, wave assigned items 31+14. Nesting guard added at the exact self-recursive call site, mirroring the sibling guards this write-up already named. See CLOSED table for full evidence; this entry's write-up (repro, fix shape, ruled-out surfaces) is kept below as the historical record, now wired into `tests/parser_nesting_gate.sh`.
...
### 24. CRASH + SILENT WRONG ANSWER: `let x: any = <bool value>` then `to_string(x)` FAILS the LLVM AOT build outright and silently misrenders on JIT (prints `1`, not `true`) (termination/types-invariant sweep, found 2026-08-05) -- **FIXED 2026-08-14, any-scalar-erasure-reject wave. A DIRECT `let x: any = <bool>` (and the same-class `let x: any = <f64>`, found while probing this item -- a `double` is no more i64-shaped than an `i1`) is now REJECTED at compile time (E0110) instead of crashing the AOT build and silently misrendering on JIT. See CLOSED table for the full root cause, fix, and proof-both-ways evidence. RESIDUAL, not covered by this fix and NOT closed: the `[any]`-ARRAY-argument variant of the exact same shape this item's own writeup names as the motivating case (`fn log_event(args: [any])` called with a bool) still silently misrenders on BOTH backends with THREE disagreeing values for `true` -- filed separately as item 40, since the compile-time type information this fix relies on is already erased before that shape's `let b: any = args[0]` is ever reached.**
...
### 40. SILENT WRONG ANSWER (backend-divergent, no diagnostic on EITHER backend): a bool routed through an `[any]` array argument disagrees with itself on THREE values across two backends (found while probing item 24; repro pre-existed on disk with no ledger entry) -- NOT FIXED

Discovered by a prior, unlogged session (`tests/security/attack_bool_any_array_backend_divergence.kry`,
whose own header comment names an "ASSAULT round 6" pass) -- the repro and
its analysis were committed to the tree but never written up here, so per
this repo's own discipline ("anything not written in the LEDGER is lost")
it is being logged now, RE-VERIFIED LIVE this session rather than trusted
from the comment. Confirmed still reproducing UNCHANGED after item 24's fix
above (expected -- the two shapes are not the same code path; see below).

Repro (`tests/security/attack_bool_any_array_backend_divergence.kry`):
```
...
```
Measured live this session, `compiler/target/release/kryos.exe`, POST the
item-24 fix: `kryos run` prints `logged: 1` / `logged: 0`; `kryos build
--release` BUILDS CLEAN (rc=0, no diagnostic at all) and the resulting
binary prints `logged: -1` / `logged: 0`. So for `true` specifically there
are THREE disagreeing values for the same source program -- correct=`true`,
JIT=`1`, AOT=`-1` -- and AOT reaches this SILENTLY (clean build, clean run,
wrong number), which is strictly worse than item 24's direct-`let` shape
(which at least failed the AOT build loudly instead of lying).

Why item 24's fix does not catch this, confirmed by construction, not just
argued: the new check (`kryos-types/src/check.rs`, `Stmt::Let`) rejects a
`Bool`/`F64`/`F32`-INFERRED initializer being stored into an explicitly
`any`-annotated `let`. In this shape the inner `let b: any = args[0]` DOES
run through that exact arm, but `args[0]`'s inferred type is already
`Type::Error` (the pre-existing `any`-erasure sentinel, item 6) by the time
it gets there -- `args: [any]`'s element type is erased at the ARRAY
PARAMETER's own declaration, before `main`'s call site or the function
body ever runs, so there is no concrete `Bool` left for the check to see.
Re-ran the exact repro above against the post-fix binary and confirmed it
still compiles clean and still misrenders (no `E0110`), not merely assumed
from reading the check's condition.

Fix shape (not attempted -- broader than item 24's scope, a separate unit
of work per this repo's own "decompose large work, verify each unit"
discipline): the check would need to run at the point a CONCRETE type last
exists before erasure -- i.e. where a bool/f64 VALUE is placed into an
`[any]`-typed array literal, or unified against an `any`/`[any]`-typed
parameter (the `[true]` array literal at the `log_event` call site, or the
parameter unification itself) -- not at the later `let` that reads an
already-erased element back out. That is a materially larger surface
(every array-literal-into-`[any]` and every call-argument-into-`any`/
`[any]` unification site, not one `Stmt::Let` arm) and needs its own
reproduction-first pass before being attempted.

Severity, per this repo's own ranking doctrine ("a silent wrong answer
outranks a crash"): this is a SILENT WRONG ANSWER with a three-way
backend-divergence twist, and ranks ABOVE where item 24 was before its fix
(item 24 at least crashed loudly on AOT; this shape's AOT path reaches a
clean build and a clean run with a wrong number, the worst case in the
ranking, not a middling one). Workaround, matches CLAUDE.md's existing
`any` gotcha: avoid routing a `bool` (or `f64`) through `[any]`/
`args: [any]` at all -- build the display string manually from the
concretely-typed value with `+`/`to_string` BEFORE it enters an
`any`-typed array or parameter position.
...
### 20. Capability checker false-rejects a closure retrieved through a generic passthrough ACCESSOR method's chained return call (`holder.get()()` where `get` just does `return self.val`) — a THIRD, deeper instance of the item-just-closed's bug class, deliberately left OPEN (combined-category grammar fuzz wave, found 2026-08-04) -- **FIXED 2026-08-14, transparent-accessor-call wave. See CLOSED table for the fix and full proof-both-ways evidence.**
...
### 12. SUPPLY CHAIN: `kryos pkg install` never reads `kryos.lock` — silently re-resolves live and overwrites the lock on every run, with no warning (RED TEAM round 1, toolchain-supply lens, found 2026-08-04) — **FIXED 2026-08-13, explicit-source wave (paired with item 17). `install()` now reads an existing `kryos.lock` and, when it covers every manifest dependency, PINS to it (fetch + checksum-verify only, no re-resolve, no lock rewrite) -- matching `npm ci`/`cargo install --locked`. This entry's repro (`tests/security/pkg_install_ignores_lock.sh`) is kept below as the historical record and was rewritten in place as a regression pin asserting the fixed behavior (still proven live, plus a new step proving `kryos pkg update` remains a deliberate escape hatch). See CLOSED table for the fix and full proof-both-ways evidence.**
...
### 21. SILENT WRONG ANSWER: a mutating closure's own mutated-scalar-capture write-back is skipped when the call that mutates it THROWS mid-body — the mutation silently vanishes instead of persisting or erroring (byproduct of closing item 11(b), found 2026-08-05) — **FIXED 2026-08-14, exception-early-return-writeback wave. Both codegen backends' post-call exception-check early-return path (synthesized entirely in codegen, never as a MIR `Terminator::Return` block) now replays the SAME mutated-scalar-capture `StoreDeref` write-back the normal-return epilogue already inserts, so a mutation made before a `throw` mid-body persists into the closure's box instead of silently reverting on the next call. See CLOSED table for the fix and full proof-both-ways evidence.**
...
### 31. PERMANENT HANG: `std::sync::Mutex.lock()`/`.unlock()` called WITHOUT reassigning the return value leaves the real native mutex locked forever, with no diagnostic (RED TEAM round 2, concurrency lens, found 2026-08-06) —  **FIXED 2026-08-14**, wave assigned items 31+14. Same-thread self-reentrancy detection added to the native mutex primitive, mirroring item 11(a)'s already-shipped closure-lock pattern -- converts the hang into a loud, attributable panic instead of eliminating the underlying non-reentrant contract. See CLOSED table for full evidence; this entry's write-up (repro, mechanism, fix-shape options) is kept below as the historical record, now wired into `tests/concurrency_smoke.sh`.
