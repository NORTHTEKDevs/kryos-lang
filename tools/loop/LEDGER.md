...
```
let a = to_array(Ok("hi-there"))          -> prints 140697945718784   (raw pointer)
let a: [str] = to_array(Ok("hi-there"))   -> prints hi-there          (correct)
```

Item 40b migrated `to_array(r: Result) -> [any]` to `to_array<T>(r: Result) -> [T]`
and its CLOSED entry asserted the unannotated call now prints `hi-there`. It does
not. That assertion has been CORRECTED IN PLACE rather than deleted, because a
verification claim in a CLOSED entry that does not reproduce is precisely what
this ledger exists to prevent, and hiding it would be worse than recording it.

MECHANISM (already stated correctly inside 40b's own entry, just not carried
through to its verification example): `to_array`'s parameter is `r: Result`, the
hand-rolled `std::result` enum whose payload is `any` -- NOT the compiler's builtin
generic `Type::Result{ok,err}`. So `T` has nothing to bind against on the argument
side; it is fixed only by an annotation at the binding site. The migration removed
`[any]` from the SIGNATURE without making an unannotated call site type-safe.

`std::iter::count<T>` is NOT affected -- its `T` binds from a real `[T]` argument.

WHY item 40/40b's container check does not catch it: that check fires at a
call-ARGUMENT boundary (a concrete `Str` landing where the parameter is the `any`
sentinel). Here the erasure is on the RETURN/binding side, a different code path,
and the argument (`Ok("hi-there")`) is a `Result` enum, not a `str`.

FIX SHAPES, none attempted:
  a. Give `std::result::Result` a real typed payload so `T` binds from the
     argument. Correct, and the largest change -- it is a hand-rolled enum used
     across the stdlib.
  b. Reject an unannotated binding whose inferred element type is the `any`
     sentinel (an inference-side sibling of the item 40/40b argument-side check).
     Needs the same container-only scoping discipline, since a bare `Type::Error`
     is also how the polymorphic builtins are typed.
  c. Document the annotation requirement on `to_array` and pin it. Cheapest and
     honest, but leaves a silent wrong answer reachable.

Severity: SILENT WRONG ANSWER, but narrow -- `to_array` had ZERO callers repo-wide
at migration time, so nothing ships broken today. Ranked here for that reason, not
because the class is benign.
