...
PeakWorkingSet64: **25MB at 500k iterations, 554MB at 3M** (~185MB/M),
confirming the original entry's number exactly. Still real, still severe.

**Why the 1st attempt's `DropIfNe` produced invalid LLVM IR, precisely.** Not
"an LLVM-only representation split" in the abstract -- the two backends map
the SAME field to different things, and both were read this session rather
than inferred:
- **Cranelift**: `mir_type_to_cl` maps `MirType::Enum(_)`/`MirType::Struct(_)`
  to `types::I64`, "pointer to heap/stack allocation". A struct field holding
  an enum stores an 8-byte POINTER to an enum box. `DropIfNe`'s existing
  I64 path therefore works here unmodified -- which is exactly why Cranelift
  "tolerated" the 1st attempt.
- **LLVM**: the enum is stored INLINE in the parent's layout, so an
  `RValue::Field` read yields a `{ i64, i64 }` aggregate. Feeding that to
  `DropIfNe`'s `old` -- declared `I64` at every existing call site, on purpose
  (see `Instruction::DropIfNe`'s own doc comment) -- is the build failure.

**The fix for the IR error: type the `old` temp as the FIELD's type, not
`I64`.** Then each backend reads the field in its own representation and
`DropIfNe`'s `ty` still carries the logical type. Tried this session: the
Cranelift side then works end to end with no backend change at all.

**MEASURED RESULT of that change (the reason it is worth continuing, and the
reason it was still not shippable):**
- JIT/Cranelift: 3M iterations went from ~600MB (at the entry's ~200MB/M) to
  **276MB** -- correct output throughout (`acc=3000` at 1k iters). Roughly
  HALF the leak, ~92MB/M remaining. Not diagnosed; do not guess at it, measure
  it. The first suspect to RULE OUT (not to assume) is the `RValue::Field`
  read's own "borrow-to-own" retain -- the Str/Array/Map path a few lines
  above emits TWO releases for exactly that reason, and this attempt emitted
  one drop.
- LLVM/AOT: still fails to build, now at the identical error one level in
  (`'%_15' defined with type '{ i64, i64 }' but expected 'i64'`), because
  `DropIfNe`'s LLVM handler itself still does `icmp ne i64 {old_val}` and
  `coerce_value(old, "i64", "ptr")` on what is now legitimately an aggregate.

**The remaining LLVM work is bounded and the address IS reachable** -- this is
the part the 1st attempt concluded was missing. `Instruction::StoreField`'s
LLVM handler already computes a real field POINTER
(`getelementptr %S, ptr {base}, i32 0, i32 {field_idx}`, with `%_N.addr` as
the base for a by-value aggregate local), and Cranelift's equivalent computes
`base + offset` from `compute_struct_layout`. So `DropIfNe` on an
aggregate-typed `old` can: extract payload word 1 for the inequality compare
(`extractvalue`), and hand a pointer to the existing `emit_enum_drop_payload`
for the drop -- either the GEP'd field address, or the aggregate spilled to an
alloca. Note the SEMANTICS differ per backend and both are correct: LLVM must
drop the PAYLOAD ONLY (the field is inline; there is no box to free), while
Cranelift must drop payload AND free the box (the field held a pointer to one).

**REVERTED** rather than shipped: a tree that does not build on AOT cannot
ship, and a half-closed leak in this subsystem risks masking the other half.
`git diff` on `lower.rs` is empty at this session's HEAD (verified after
revert: full `cargo build --release` clean, AOT build of this item's own
fixture rc=0).

#### 3rd pass (2026-08-29, same session): HALF of this leak was never item 51's at all -- it was item 54, now FIXED

Bisecting the residual produced the finding that a plain ARRAY field, with no
enum anywhere, leaked at the same ~92MB/M rate. That turned out to be a
SEPARATE mechanism -- `Instruction::StoreField` was missing from
`drop_unescaped_str_temps`'s whole-window allowlist, so a field assignment
aborted the temp-cleanup pass for the entire statement -- and it is now closed
as **item 54** (see the CLOSED table). Fixing it took THIS item's own repro
from **554MB to 279MB at 3M iterations**, measured, because the array-literal
payload temp feeding `Val.ListV([i, i+1, i+2])` was one of the intermediates
the guard had been suppressing.

**What remains of item 51 is ~92MB/M, and it is NOT a missing drop -- it is a
deliberate, documented tradeoff whose root is item 3.** Pinned down by
elimination, every leg measured at 3M iterations: a mutable struct read in a
loop with no reassignment is flat; reassigning field X while reading a
DIFFERENT field Y is flat; only reassigning field X while READING field X
leaks, at almost exactly one small array per iteration (~30 bytes). The cause
is stated in `lower.rs`'s own comment on the `field_source` branch: **a field
read of a MUTABLE-CONTAINER field (array/map/struct) is treated as a BORROW
and deliberately NOT dropped**, because `push` grows the shared buffer in
place, so dropping such a temp releases a buffer the struct still points at --
"5 of the 7 double-frees and a SEGFAULT in stage1_mini_parser.kry". The read
therefore retains a reference that nothing balances, and when the slot is
later overwritten the release cannot reach zero.

Distinguishing a genuine borrow (read, mutate in place, struct keeps owning)
from an owning read whose slot is about to be replaced is ALIAS ANALYSIS --
the same unified-ownership work item 3 has concluded twice that it needs. So
item 51's residual is not an independent bug to be patched; it closes when
item 3 closes, and an 11th attempt at either should treat them as one piece of
work. (A caution for whoever measures this: reading the SAME container every
iteration keeps RSS flat even when refcounts grow without bound, because
nothing new is allocated -- two of the flat legs above are flat for exactly
that reason and prove nothing on their own. This is the same measurement trap
this file's `field_source` comment already records being burned by once.)

Found while enumerating item 49's adjacent shapes before closing it (rule 4).
`h.v = Val.ListV([i, i+1, i+2])` in a loop leaks on BOTH backends: ~185MB/M
AOT, ~200MB/M JIT (`tests/mem/struct_field_enum_overwrite_leak.kry`, 500k/3M
iters). Same root cause as item 49's now-fixed array/map shapes
(`release_if_ne_fn` is None for Struct/Enum), but NOT fixable by the same
`Instruction::DropIfNe` mechanism as-is: an array/map ELEMENT is always a
boxed i64 pointer on both backends (read via the raw `kryos_array_get`/
`kryos_map_get` getters, which is what let `DropIfNe` compare/free it as a
plain handle) -- a Struct/Enum-typed struct FIELD is embedded INLINE in the
parent struct's layout on LLVM instead. CONFIRMED by trying it and
reverting: assigning an `RValue::Field` read of such a field into a
plain-I64 `DropIfNe` `old` local produced invalid LLVM IR ("defined with
type `{ i64, i64 }` but expected `i64`"), a BUILD FAILURE caught before it
ever reached a gate, not a runtime bug -- reverted rather than shipped
broken. Cranelift tolerated the identical code (it represents Struct/Enum
uniformly as a pointer everywhere), so this is an LLVM-only representation
split, mirroring the aggregate-vs-pointer divergence
`kryos-mir/src/lower.rs` already works around elsewhere for struct-field
mutation through an array index (see the `Ptr(elem_ty)` comment in
`lower_nested_field_assign`'s `IndexAccess` arm). Closing this needs a
DIFFERENT mechanism than `DropIfNe`'s raw-pointer-inequality guard -- an
aggregate-aware compare/drop, or spilling the embedded value to a heap box
before comparing. Reproduce: `tests/mem/struct_field_enum_overwrite_leak.kry`.

### 46. AVAILABILITY: three independent concurrency-primitive hangs/deadlocks, found as red-team probes that sat untracked in the working tree, never logged here or run in CI (hygiene wave, 2026-08-28) -- ALL THREE FIXED (2026-08-28 follow-up session) -- ITEM CLOSED, moved to the CLOSED table
...
**2026-08-28 FOLLOW-UP SESSION -- ALL THREE FIXED, see the CLOSED table entry
at the top of this file for the full evidence (repro before/after, fails-
without-fix proof for each, adjacent shapes enumerated and tested, gate
output). One-line summary of each fix:**

- **(a) `wg_wait` starvation**: `wg_done` (`compiler/stdlib/chan.kry`) now
  CLOSES `done_ch` instead of sending one token -- close is a real broadcast
  (`notify_all` + every later `recv` on a closed/drained channel returns
  immediately), so all waiters wake, not just one.
- **(b) cross-closure AB-BA**: `kryos_closure_lock_acquire`
  (`compiler/crates/kryos-stdlib-native/src/sync_prims.rs`) now maintains a
  small cross-thread wait-for graph and detects a circular wait of ANY
  length (2-cycle AB-BA, 3-cycle, ...) while spinning, failing loudly via
  `kryos_panic` (naming the full thread/lock chain) instead of hanging
  forever. A global lock-ORDERING fix was considered and rejected: the
  codegen-inserted acquire happens one call at a time as the dynamic call
  graph unfolds, so there is nothing to sort ahead of time.
- **(c) coop-yield-while-locked**: the same spin loop now yields the COOP
  BATON (`kryos_coop_yield`, via `kryos_rt::executor`), not just the OS
  thread, on every failed attempt while running on a cooperative task --
  a no-op on a plain OS `spawn` thread, so item 7b's existing plain-thread
  serialization guarantee (`tests/conformance/conf_spawn_closure_capture_lock.kry`,
  50 threads x 2000 calls) is untouched.

All three gated permanently in `tests/concurrency_smoke.sh` (part of tier 1,
`tools/loop/kryos-loop.sh gates`), including 4 new adjacent-shape regression
files: `attack_cross_closure_lock_3cycle_deadlock.kry` (3-cycle, not just
AB-BA) and `attack_closure_lock_coop_direct_yield_deadlock.kry` (explicit
`coop_yield()` yield point, not just `sleep_ms`), plus an inline 5-waiters-
plus-late-arrival shape for (a) and the pre-existing coop control file wired
in for the first time. `tests/security/attack_chan_waitgroup_multi_waiter_hang.kry`
and `attack_closure_lock_coop_yield_deadlock.kry` (the original two repro
files from this item) are now gated too -- they were not before.
...
| Item | Evidence |
| --- | --- |
| **item 54 (NEW, found 2026-08-29 while diagnosing item 51's residual): a struct-FIELD assignment whose right-hand side builds INTERMEDIATE heap temps (`s.name = "v-" + to_string(i)`) leaked every intermediate on both backends, ~61MB/M -- FIXED, after the first version of the fix broke a pinned alias-refcount output and had to be narrowed** | Closed 2026-08-29. FOUND, not reported: while bisecting why item 51's leak survived at ~92MB/M, a plain ARRAY field with no enum anywhere leaked at the same rate, which meant a second mechanism was in play. ISOLATED BY A 2x2 before any fix, each leg measured at 3M iterations on AOT -- the same concat assigned to a plain LOCAL: **flat, 3MB**; a field store of a bare LITERAL (no intermediates): **flat, 3MB**; a field READ in a loop with no store: **flat, 3MB**; a field store WITH intermediates: **LEAKS, 184MB** (32MB at 500k, ~61MB/M). Only the combination leaks, which is why every single-variable test missed it. ROOT CAUSE: `drop_unescaped_str_temps` (`kryos-mir/src/lower.rs`) guards its whole window with a `_ => return` catch-all over instruction shapes it does not model, and `Instruction::StoreField` -- what a field assignment lowers to -- was never on the allowlist, so the pass ABORTED FOR THE ENTIRE STATEMENT and dropped none of its temps, including the `to_string(i)` intermediate feeding the concat that is actually stored. This is exactly item 49's failure mode one statement shape over: that item had to add `Instruction::DropIfNe` to the same list for the same reason, and its own comment says so. FIX: add `Instruction::StoreField` to the guard, and -- since the guard was blunt precisely because an unmodelled shape can hide an escape -- teach the per-candidate walk that a candidate appearing as a StoreField's `value` or `object` ESCAPES (the value is MOVED into the field; the field's own `release_if_ne` pair handles the old occupant). **THE FIRST VERSION OF THIS FIX WAS WRONG, AND A PINNED TEST CAUGHT IT.** Relaxing the guard also let the `field_source` branch drop a field-READ temp inside a field-assignment window. For a SELF-REFERENTIAL assignment (`a.tag = a.tag + "!"`) that lands on top of the field-assign path's own accounting -- which already reads the old field and emits a PAIR of `release_if_ne` calls, the first balancing that read -- and over-releases a string other aliases of the same struct box still point at. `tests/security/attack_container_element_alias_refcount.kry`'s pinned AOT output went from `x19999!|x19999|x19999|x19999|5` to `x19999!|x19999!||x19999!||x19999!||5`: three aliases of one box flipped from reading the unmutated value to reading the mutated one with empty strings spliced in. That is CORRUPTION, not a leak -- the dangerous direction for this pass, and the same direction its comments record breaking the self-host parser before. NARROWED FIX: field-READ temps are held back whenever the window contains a `StoreField` (`window_has_store_field`); pure intermediates take the `owns` path and are unaffected, which is all item 54 needed. VERIFIED after narrowing: the alias-refcount pin holds on BOTH backends again (`x19999!|x19999!|x19999!|x19999!|5` JIT, `x19999!|x19999|x19999|x19999|5` AOT) AND the leak stays closed (184MB -> 5MB at 3M; the gate reads 4MB AOT / 3MB JIT at 5M). GATED: `tests/mem/field_assign_temp_leak.kry` + `tests/mem_field_assign_temp_gate.sh` (NEW, both backends, 5M iters, 50MB ceiling, wired into `.github/workflows/ci.yml` beside the item-45/49/52 gates). The gate deliberately measures only the LEAK direction; the DOUBLE-FREE direction it cannot see is covered by `tests/no_double_free.sh`, `tests/backend_divergence_pins.sh`, the self-host bootstrap and `compiler/self-host/test_regressions.sh` -- and those, not the ceiling, are what caught the first version. NOTE, disclosed: `tests/fixtures_tracked_gate.sh` failed on the first ladder run because the new fixture was written but not `git add`ed -- the gate doing exactly its job, on this session's own work. SIDE EFFECT ON ITEM 51, measured not assumed: this fix alone took item 51's own repro from **554MB to 279MB at 3M** (its array-literal payload temp was one of the intermediates the guard was suppressing). The remaining ~92MB/M is a DIFFERENT and already-documented mechanism -- see item 51's addendum. |
| **item 53: a module path with MORE than two segments (`std::result::to_array(r)`) did not parse at all -- `E0003: unexpected token '::', expected expression` -- even though the `use std::result::{to_array}` import that makes the short spelling available is itself written with the full path -- FIXED** | Closed 2026-08-29, same session it was logged. REPRODUCED FIRST on the pre-change binary: `let a: [i64] = std::result::to_array(r)` -> `E0003` with the span on the SECOND `::`, while both `to_array(r)` and `result::to_array(r)` compiled and ran. ROOT CAUSE: `kryos-parser`'s `Ident` primary-expression arm consumed exactly ONE `::` segment pair (`name` then `method`) and then looked for `(`; a third segment left the next `::` unconsumed for the expression parser to choke on. FIX (`compiler/crates/kryos-parser/src/parser.rs`, that one arm): loop while another `::` follows, shifting `type_name = method; method = next`. Kryos's module namespace is FLAT -- module-level functions are registered under their plain name and MIR resolves a qualified call by its module-alias segment -- so only the last two segments carry meaning: the final one is the item, the one before it the module alias, and any leading segments are a qualification prefix. `std::result::to_array(r)` therefore means exactly what `result::to_array(r)` already meant. STRICTLY ADDITIVE, which is the whole safety argument: the new loop only executes when a THIRD segment is present, and that was a hard parse error before, so no program that parsed before parses differently now. RULE 4 (adjacent shapes, all run live): two-segment path to a GENERIC stdlib fn (the reported case) works and agrees BY VALUE with the one-segment spelling; two-segment path to a NON-generic stdlib fn (`std::strext::to_upper("ab")` -> `AB`) works, so the fix is not coupled to the monomorphization arm; a THREE-segment path (`deep::std::result::to_array(r)`) resolves correctly by the same last-two rule; `Enum::Variant` in expression position (`Col::Green`) is unchanged; `Type::method(args)` static dispatch (`P::make(11)`) is unchanged. One probe of mine failed and was MY error, not the compiler's -- `std::strext::repeat_str` does not exist, and the compiler said so precisely (`E0204: module 'std::strext' has no export 'repeat_str'`); rerun against a real symbol, it passes. PROVEN BOTH WAYS: `git stash` the parser change alone, full `cargo build --release`, rerun -> `E0003` at the same span (plus a cascading `E0009`); `stash pop`, rebuild -> PASS. GATED: `tests/conformance/conf_qualified_generic_call.kry` (item 50's own program, the natural home since it already owns this subject) gained cases 7 and 8 -- the two-segment generic path checked BY VALUE against case 1's result rather than merely for parsing, plus the one- and two-segment non-generic pair -- running on BOTH backends. EVIDENCE: full `cargo build --release` clean; the extended conformance program PASS on Cranelift JIT and LLVM AOT; `tools/loop/kryos-loop.sh gates 2` tier 1 all PASS with conformance 68/68 and `tier2 GREEN`; self-host bootstrap 16/16 ALONE; `tests/ecosystem_check.sh` 259/259. NOTE ON THE DIFF: the repo's text-sanitizer hook normalised em-dashes to hyphens in comments throughout `parser.rs` when it rewrote the file, so the commit's line count is much larger than the change -- only three non-comment lines differ, all of them this fix. |
| **item 52: LEAK -- an Enum construction with a STR payload field (`Enum.V(str_expr)`) orphaned the pre-clone string on every construction, push or overwrite, ~78MB/M on both backends -- FIXED; the first version of the fix was a USE-AFTER-FREE that the conformance suite caught, and closing it properly also closed a latent hole item 45 had carried since it shipped** | Closed 2026-08-29. REPRODUCED FIRST (rule 1), live, on the fixture the prior session pinned (`tests/mem/enum_str_push_leak.kry`), before touching anything: AOT peak RSS **43MB at 500k iterations and 234MB at 3M** -- ~78MB/M, growing with the iteration count, which is what makes it a leak and not a fixed allocation, and matching the OPEN entry's own number exactly. ROOT CAUSE, read in both backends end to end rather than inferred from the struct path: `RValue::EnumVariant`'s payload-store switches on the VARIANT'S DECLARED FIELD TYPE -- `Some(MirType::Array(..)) => kryos_array_dup`, `Some(MirType::Str) => kryos_string_clone`, `_ => val` (a plain MOVE) -- in Cranelift (`codegen.rs`, the payload-store match) and identically in LLVM (`Some(MirType::Str) if val_ty == "ptr"` -> `call ptr @kryos_string_clone`). Item 45 gave `drop_unescaped_str_temps` (`kryos-mir/src/lower.rs`) the matching `RValue::EnumVariant`-with-ARRAY-field arm and never the Str one, so for a str payload the enum took a CLONE, the source temp kept the ORIGINAL, and nothing dropped it. The asymmetry with `RValue::Struct` -- whose arm is correctly Array-only -- is real and load-bearing: a struct literal MOVES a str handle in with a bare insertvalue, so dropping the temp there would be a double free. FIX: `kryos-mir/src/lower.rs` only, widening that one arm. **THE FIRST VERSION OF THIS FIX WAS WRONG, IN THE DANGEROUS DIRECTION, AND IS THE REASON THIS ENTRY IS WORTH READING.** Keying the arm off the candidate TEMP's type (`matches!(cand_ty, Str \| Array)`) passed every check I had written for it -- the leak gate went 234MB -> 3MB on both backends, seven hand-written adjacent-shape probes (computed payload, literal payload, two str fields, str-beside-array, returned-from-fn, inside a struct field, Map negative control) all returned correct values on BOTH backends, and `KRYOS_FREE_DIAG=1` was silent -- and it was still a USE-AFTER-FREE. A monomorphized GENERIC enum (`Box<str>`, `Box.Full("item-" + to_string(i))`) has a str-typed TEMP whose DECLARED field type is not `MirType::Str`, so both backends fall to `_ => val` and MOVE the handle while the pass dropped it, freeing the payload before the very next `match` arm read it. Caught by the existing `tests/conformance/conf_generic_enum.kry` ("heap-enum match loop correct (payload retain/release balanced)") going red in the full ladder -- by the gate ladder, that is, not by any probe written for this item, which is the whole argument for running it before believing a memory fix. CORRECTED FIX: the arm now resolves the enum's DECLARED payload type via `ctx.enum_defs[enum_name][variant_idx].fields[i]` and requires it to match the temp (`Some(Str)` with a Str temp, `Some(Array)` with an Array temp) at EVERY slot the temp occupies (`Pair.P(s, s)` -- one moved slot poisons the whole candidate), bailing otherwise. That is precisely the condition the backends switch on, so the pass can no longer disagree with codegen. **The ARRAY half carried the identical latent hole from item 45 onward and is closed by the same change** -- a generic enum with an array payload would have been a use-after-free today; being wrong in the bail direction costs a leak, being wrong in the other costs memory corruption. PROVEN BOTH WAYS, twice over: (1) the leak -- `git stash` the lower.rs change alone, full `cargo build --release`, `tests/mem_enum_str_push_gate.sh` -> **AOT 388MB and JIT 388MB at 5M iters, both legs FAIL "leak reintroduced"**; `stash pop`, rebuild -> AOT 4MB, JIT 3MB, PASS. (2) the use-after-free -- the arm was temporarily reverted to the naive temp-type condition and rebuilt, and BOTH `conf_enum_str_payload.kry` (`FAIL: generic enum str payload readback, got 0` -- 0, i.e. the payload was already freed) and `conf_generic_enum.kry` went red; restored and rebuilt, both PASS. GATED, two complementary gates because a memory ceiling and a value check cannot see each other's failure: `tests/mem_enum_str_push_gate.sh` (NEW, modelled on item 45's gate, both backends, 5M iters, 50MB ceiling, wired into `.github/workflows/ci.yml` beside the item-45 and item-49 gates) measures the LEAK; `tests/conformance/conf_enum_str_payload.kry` (NEW, auto-discovered, value-asserting on BOTH backends) measures the OTHER direction -- that dropping the temp does not free what the enum kept -- across the computed-payload push-and-readback loop, a literal payload, two str fields, a str field beside an array field, a returned enum, an enum inside a struct field, a Map payload as the MOVED negative control (the case that double-frees if the arm is ever widened to every heap payload kind), and case 8, the monomorphized-generic boundary that this fix's first version got wrong, pinned from this item's own side so a future widening cannot pass its own gate while breaking someone else's. EVIDENCE, all fresh: full `cargo build --release` clean (no `-p`); `tests/mem_enum_str_push_gate.sh` PASS 4MB/3MB; `tests/mem_enum_array_push_gate.sh` (item 45) PASS 4MB/3MB and `tests/mem_enum_overwrite_gate.sh` (item 49) PASS on all four legs, both re-run AFTER the declared-type tightening to prove it did not make either sibling fix vacuous; `tools/loop/kryos-loop.sh gates 2` tier 1 all PASS with conformance **68/68** and `tier2 GREEN`; `tests/security_gate.sh` PASS; `compiler/self-host/test_bootstrap.sh` 16/16 ALONE; `tests/ecosystem_check.sh` 259/259. |
| **item 50: a module-qualified call to a module-level GENERIC function (`result::to_array(r)`) passed `kryos check` clean and then died at BUILD time with an unresolved symbol on both backends -- FIXED, in two halves, the second of which was a silent-wrong-answer regression the first half would otherwise have SHIPPED** | Closed 2026-08-29. REPRODUCED FIRST (rule 1), live, on a binary rebuilt with the fix stashed out: `let a: [i64] = result::to_array(r)` -> `kryos check` rc=0 CLEAN, then `kryos run` (Cranelift) `LNK1120: 1 unresolved externals` and `kryos build --release --backend llvm` `use of undefined value '@to_array'` -- matching the OPEN entry's reported signature exactly, on both backends, with the SAME call spelled unqualified linking and running fine. ROOT CAUSE (read, not guessed): `kryos-mir/src/lower.rs` had TWO call-lowering arms that can name a module-level generic. The `FnCall` arm (`to_array(r)`) consulted `ctx.generic_templates` and monomorphized; the `StaticMethodCall` arm (`result::to_array(r)`) resolved the module alias to the plain function name and then emitted `RValue::Call` against that BARE TEMPLATE name, which no backend ever codegens -- hence a symbol that type-checks and does not exist. A second, independent half of the same split lived in `infer_expr_type`: its `StaticMethodCall` arm looked the return type up in `func_ret_types`, which is keyed by the MONOMORPHIZED name, so a qualified generic call inferred `Void` -- an unannotated `let pairs = iter::zip(a, b)` then lost its element type and failed codegen on BOTH backends (`cannot resolve the struct type for field access '.0'` / `extractvalue operand must be aggregate type`). FIX: both lowering arms now route through one shared `lower_generic_fn_call`, and both inference arms through one shared `infer_generic_fn_call_ret`, specifically so they cannot drift apart again; the qualified-inference guard is scoped to a MODULE qualifier (`type_name` is not a known struct/enum), and generic IMPL methods are unaffected (they live in a separate `generic_impl_fn_templates` map, matched and returned earlier in the same arm). SECOND HALF -- THE REGRESSION THE FIRST HALF WOULD HAVE SHIPPED (rule 4, enumerate the adjacent shapes): with only the MIR fix in the tree, `let a = result::to_array(Ok("hi-there"))` -- previously a loud LINK FAILURE -- now LINKED and printed a raw pointer (`140699791077376`), `kryos check` rc=0 clean. That is item 48's exact silent-wrong-answer class, arriving through a call shape item 48 never covered, and by this repo's own ranking doctrine a silent wrong answer outranks the build failure the fix was closing: the first half alone was a NET SEVERITY REGRESSION. Cause: `report_unbindable_generic_call` (`kryos-types/src/check.rs`, item 48's E0110 check) only matched `Expr::FnCall` over an `Identifier`; the `::` spelling is an `Expr::StaticMethodCall` and never reached it -- a gap that was UNREACHABLE until the MIR half made the shape link at all. FIX: that function now resolves BOTH spellings to a module-level function name and runs the identical signature-shape predicate, returning early when `type_name` names a known struct, enum, or trait (a `Type::method(..)` static call is not a module-level call, and erring toward NOT reporting is the safe direction for a name collision). VERIFIED CONVERGENCE, not merely "rejected": the two spellings now agree on every probed shape -- including `let a = result::to_array(r)` with `r: Result<i64, str>` ANNOTATED, which is also E0110 unqualified (pre-existing, untouched code path, confirmed live) because `to_array`'s `T` appears in no parameter type and so is unbindable from any argument at any call site; it had been printing `42` only because i64 coincides with the erased representation. PROVEN BOTH WAYS, separately per half: (1) MIR half -- `git stash` lower.rs alone, full `cargo build --release`, both minimal repros RED on both backends with `check` rc=0 (messages above); `stash pop`, rebuild, both GREEN. (2) check.rs half -- `git stash` check.rs alone, full rebuild, `tests/type_soundness.sh` -> exactly the 3 new `want_reject` probes report `HOLE ... unsound program passed kryos check` and all 6 new `want_pass` probes stay green (so the red is the fix's absence, not a probe artifact); `stash pop`, rebuild, `type-soundness: all probes correct`. GATED, two independent gates: `tests/conformance/conf_qualified_generic_call.kry` (NEW, auto-discovered by `run_conformance.sh`, runs on BOTH backends) is VALUE-asserting, not build-only -- it checks every result and `exit(1)`s on mismatch, and covers the reported shape, its `Err` arm, the unqualified reference spelling, a SECOND instantiation at a different `T` through the qualified arm (pins per-T mangling against a shared-name cache bug), a different module (`iter::count`, proving the fix is not result-specific), and a two-type-param generic (`iter::zip`) whose tuple elements are read back positionally (the inference half). `tests/type_soundness.sh` gained 3 `want_reject` probes (`to_array_unannotated_qualified_call`, `..._qualified_tuple_destructure`, `..._qualified_typed_arg`) and 6 `want_pass` no-cascade complements (annotated qualified call; qualified BINDABLE generic unannotated; qualified two-param generic return-inference; qualified call in a struct-literal field; qualified call as a concrete-typed call argument; plus the existing unqualified twins), each `want_pass` runtime-verified rather than merely accepted. EVIDENCE, all fresh this session: full `cargo build --release` clean (no `-p`, per rule 2), one PRE-EXISTING unrelated warning (`tail_value_is_identifier` dead code in `kryos-mir`, untouched by this change); the new conformance program PASS on Cranelift JIT and on LLVM AOT; `tests/type_soundness.sh` all probes correct, run both red-without-fix and green-with-fix; `tools/loop/kryos-loop.sh gates 2` rc=0 -- tier 1 ALL PASS (conformance **67/67**, both backends) and `tier2 GREEN`, matching the documented baseline with no drop; `tests/security_gate.sh` PASS; `compiler/self-host/test_bootstrap.sh` run ALONE per doctrine, **16/16 PASS**; `tests/ecosystem_check.sh` **259/259 clean**. The last two matter specifically for the check.rs half: a false-positive E0110 would fire across 22,893 lines of self-host Kryos and 259 ecosystem programs, so their green is the no-cascade evidence for the new arm, not a formality. NOT FIXED / DISCLOSED, not silently dropped: a TWO-segment path in expression position (`std::result::to_array(r)`) still does not parse -- `E0003: unexpected token '::', expected expression` -- an honest check-time diagnostic with a correct span rather than a silent wrong answer or a build failure, so it was left out of scope and logged as its own OPEN item 53 instead of being fixed opportunistically. |
| **item 46: AVAILABILITY -- three independent concurrency hangs/deadlocks reachable from ordinary non-adversarial usage: (a) `std::chan::ChanWaitGroup` starved every `wg_wait` caller past the first (a `send` token is a queue, not a broadcast), (b) an AB-BA lock-ordering deadlock between two DIFFERENT mutating closures' locks was invisible to the existing self-reentry detector, (c) the codegen-inserted closure-call lock's bare CAS spin had no knowledge of the cooperative executor and could wedge the coop baton forever if held across a coop-yield point -- ALL THREE FIXED** | Closed 2026-08-28. REPRODUCED FIRST (rule 1) on the pre-fix release binary, all three, `kryos run`, `timeout 12/15`, rc=124 (still alive past timeout) for each -- matching each file's own predicted hang exactly. **(a)** `compiler/stdlib/chan.kry::wg_done` changed from `send(wg.done_ch.raw, 1)` to `close_chan(wg.done_ch.raw)` on the count-reaches-zero branch: `kryos_chan_close` (kryos-rt/src/channel.rs) sets the closed flag and calls `notify_all`, a real broadcast, and the documented closed-channel contract (a `recv` on a closed, drained channel returns immediately) covers any later-arriving waiter too, so `wg_wait`'s existing `recv(wg.done_ch.raw)` needed no change. BEFORE: 2-waiter repro hung, rc=124. AFTER: both waiters report completion, rc=0, reran 20/20 clean. Reverted the one-line fix, rebuilt nothing needed (pure stdlib .kry), reran -- hung again, rc=124, confirmed fails-without-fix; reapplied. **(b)+(c)** both fixed in `compiler/crates/kryos-stdlib-native/src/sync_prims.rs`'s `kryos_closure_lock_acquire`/`kryos_mutex_lock`, sharing a new `spin_acquire(km, on_stall)` helper (the plain `std::sync::Mutex` path passes a no-op `on_stall`, so its spin behavior is byte-for-byte unchanged). **(b)**: a `ClosureLockWaitGraph` (`owners: HashMap<usize, ThreadId>`, `waiting: HashMap<ThreadId, usize>`, one `Mutex`-guarded global via `OnceLock`) is updated at acquire/release; `check_closure_deadlock` walks `waiting_for_key -> owner -> owner's waiting_for_key -> ...` (bounded 4096 hops), throttled to once per ~25ms of real stall time via `Instant`/`Duration`, and on finding the chain close back on the starting thread calls `kryos_panic` naming the full chain (thread ids + lock addresses). Not a false-positive risk: a `waiting` entry exists only while a thread is genuinely inside this exact spin loop, so a detected cycle is a live Coffman circular-wait, not a heuristic. A global lock-ORDERING fix (acquire low->high) was considered and rejected: the acquire happens the instant a mutating closure's thunk is entered, one call at a time, so by the time thread 1 is inside A's lock it does not yet know it will want B's too -- there is nothing to sort ahead of time, the acquisitions are inherently nested/dynamic, not a batch. **(c)**: `spin_acquire`'s `on_stall` hook for the closure lock now calls `kryos_coop_yield` (via `kryos_rt::executor::{kryos_coop_current, kryos_coop_yield}`) whenever `kryos_coop_current() != 0` (i.e. running on a coop task) -- a no-op on a plain OS `spawn` thread. This hands the COOP BATON back to the scheduler on every failed CAS attempt instead of only yielding the OS scheduler, letting the scheduler resume whichever other task can make progress, including eventually the lock holder once its own blocking op finishes and it re-queues itself. BEFORE (both b and c, same unfixed binary): `attack_cross_closure_lock_deadlock.kry` and `attack_closure_lock_coop_yield_deadlock.kry` both hung, rc=124. AFTER (full `cargo build --release`, rule 2 honored -- 4 full release rebuilds total this session across the revert/reapply cycles, ~35-90s each): (b) now panics cleanly, rc=98, message names both threads and both lock addresses; (c) now completes, rc=0, `order: A=1 B=3` matching the file's own documented control output exactly. Reverted `sync_prims.rs` to its pre-session content via `git stash`, full release rebuild, reran (b)+(c)+both new adjacent-shape files -- all four hung, rc=124, confirmed fails-without-fix; `git stash pop` restored the fix, rebuilt, reconfirmed all four pass. Each of the 3 fixed repros reran 20/20 with no flake. RULE 4 (adjacent shapes): enumerated for (a) -- 3+ waiters, a late waiter arriving after `wg_done` (the pre-existing `c<=0` fast path), waiters spawned via a loop/different binding form, multiple independent WaitGroups, incremental `wg_add` -- TESTED 3+ waiters via a dynamic loop plus a late arrival in one combined shape, gated inline in `concurrency_smoke.sh` (`chan_wg_5_waiters_plus_late`). For (b) -- 3-cycle A->B->C->A, 4+-cycle, closures in an array instead of a map, indirect calls through a helper fn, both backends -- TESTED the 3-cycle (new file `attack_cross_closure_lock_3cycle_deadlock.kry`, reproduced hanging on the unfixed binary and passing/panicking correctly on the fixed one, 2 distinct closure-lock addresses correctly named per cycle position). For (c) -- a channel op instead of `sleep_ms` as the yield point, 3+ coop tasks, both backends -- TESTED an explicit `coop_yield()` call in place of `sleep_ms`/`io_offload` (new file `attack_closure_lock_coop_direct_yield_deadlock.kry`, same before/after proof). NOT tested as a literal channel-op adjacent shape: `recv(ch)` is NOT itself coop-aware today (`kryos_chan_recv`, kryos-rt/src/channel.rs, blocks on a plain `Condvar` with no `io_offload`/`kryos_coop_io_begin` call at all) -- that is a separate, pre-existing gap in the channel implementation's coop-integration, not part of this item's closure-lock defect, and is left OPEN/undiscovered by this session's brief (not filed as a new ledger item this session -- flagged here for whoever picks it up next). GATES: all three fixes plus both new adjacent-shape files plus the two original repro files (neither was gated before this session) wired into `tests/concurrency_smoke.sh`; full `bash tools/loop/kryos-loop.sh gates 2` run: tier 1 21/21 PASS (conformance 65/65, concurrency_smoke PASS with all new checks), tier 2 7/7 PASS, `tier2 GREEN` -- matches the documented baseline, no regression. `bash compiler/self-host/test_bootstrap.sh` run ALONE: 16/16 PASS. `tests/conformance/conf_spawn_closure_capture_lock.kry` (item 7b's own 50-threads x 2000-calls race-freedom property) reran clean, 1.26s, confirming the closure-lock changes did not regress plain-thread serialization. Files: `compiler/stdlib/chan.kry`, `compiler/crates/kryos-stdlib-native/src/sync_prims.rs`, `tests/concurrency_smoke.sh`, new `tests/security/attack_cross_closure_lock_3cycle_deadlock.kry` + `tests/security/attack_closure_lock_coop_direct_yield_deadlock.kry`. |
| **item 47: RESOURCE-DOS -- `monomorphize_struct`/`monomorphize_enum` had no `enter_mono_frame`/`MAX_MONO_TOTAL`/`MAX_MONO_DEPTH` guard, unlike `monomorphize`/`monomorphize_impl_fn` -- a small, idiomatic self-referential generic (struct OR enum) hung `kryos run`/`kryos build` indefinitely with unbounded memory growth -- FIXED** | Closed 2026-08-28. REPRODUCED FIRST (rule 1), live, on the pushed pre-fix release binary before touching anything: `kryos check tests/security/attack_mono_struct_growing_selfref_no_depth_guard_hang.kry` -> rc=0 in 0.55s, zero diagnostics (well-typed; the hang is not a type error). `kryos run` on the SAME file, run detached and polled (a shell-level `timeout` wrapper does not reliably kill this binary's process tree on this machine -- confirmed separately): still running after 15s with RSS 5.59GB, growing to 8.22GB by 23s, zero stdout throughout -- killed manually (`taskkill /F /IM kryos.exe /T`). ROOT-CAUSED by reading (not guessed), matching the OPEN entry's own analysis exactly: `enter_mono_frame`/`exit_mono_frame` (`compiler/crates/kryos-mir/src/lower.rs`, defines `MAX_MONO_DEPTH=300` and `MAX_MONO_TOTAL=200_000`) are called from exactly 2 sites -- `monomorphize` (~16308) and `monomorphize_impl_fn` -- never from `monomorphize_struct` (~15688) or `monomorphize_enum` (~15762). Both functions DO have a placeholder-insert guard (`ctx.struct_defs.insert(mangled, Vec::new())` / `ctx.enum_defs.insert(...)`) that correctly short-circuits an EXACT-same-mangled-name self-reference (`Node<T> { next: Option<Node<T>> }`), but a struct/enum whose recursive field wraps the type parameter in ANOTHER generic at each level (`Node<T> { next: Option<Node<Wrap<T>>> }`) produces a DIFFERENT mangled name every recursion, so the placeholder never matches and `substitute_type_expr_to_mir`'s synchronous recursive call back into `monomorphize_struct`/`monomorphize_enum` (confirmed by reading `substitute_type_expr_to_mir`'s `TypeExpr::Generic` arm, which calls straight back into whichever of the two functions owns the name) had no depth or breadth cap at all. FIX: added the identical `enter_mono_frame(ctx, name, &mangled)` / `exit_mono_frame(ctx)` pair used by `monomorphize`, placed in both functions right after the existing cache-check (so a genuine cache hit -- including the exact-self-ref placeholder case -- still returns early without consuming a frame, exactly matching `monomorphize`'s own pattern) and right before the final `return mangled` (after the real definition overwrites the placeholder). `compiler/crates/kryos-mir/src/lower.rs` only, 4 new lines of logic (2 `enter_mono_frame` + 2 `exit_mono_frame` calls) plus explanatory comments. CAP VALUE: reused the EXISTING `MAX_MONO_DEPTH=300` / `MAX_MONO_TOTAL=200_000` constants rather than choosing new ones -- these already bound `monomorphize`/`monomorphize_impl_fn` and are documented in-file as "well above any real generic instantiation chain" (the self-host compiler's own deepest generic nesting is under 20 levels); the self-host bootstrap (below) is the live proof this remains true for struct/enum monomorphization too. PROVEN BOTH WAYS (rule 3), TWICE (once for the struct shape, once independently for the adjacent enum shape): (1) stashed the `lower.rs` fix out via version control's stash mechanism, full `cargo build --release` (rule 2 -- `kryos-mir` links into `kryos_rt`/`kryos_stdlib_native`), reran the struct repro detached/polled -- hangs again, RSS 4.93GB and climbing at 12s, zero stdout (fresh reproduction, not cached); restored the fix, full rebuild, reran -- `kryos run` now exits rc=1 in 0.28-0.33s with a clean `error[E0113]: monomorphization limit exceeded: generic \`Wrap\` recursively monomorphized to a depth of 301 (max 300)` naming the offending type and printing the full instantiation chain (`Node___i64 -> Node___Wrap___i64 -> ...`). (2) Independently repeated the identical stash/build/revert/build/restore cycle for the NEW enum-shape adjacent test (`attack_mono_enum_growing_selfref_no_depth_guard_hang.kry`, written this session) -- reverted binary: hangs, RSS 3.88GB and climbing at 15s, zero stdout; fixed binary: rc=1 in 0.26s, same `mono_fatal` E0113 diagnostic, chain named `EBox___i64 -> EBox___Wrap___i64 -> ...`. Both revert cycles confirmed via an EMPTY pending working-tree change on the touched file immediately after each stash-out, and a 4-count grep for `enter_mono_frame`/`exit_mono_frame` occurrences immediately after each restore (matching the intended 2+2 insertion). RULE 4 (adjacent shapes enumerated): (a) growing ENUM twin of the struct repro (`monomorphize_enum`'s own, separate call site) -- REPRODUCED the original hang pre-fix, FIXED post-fix, both proven both-ways above; new gate file `tests/security/attack_mono_enum_growing_selfref_no_depth_guard_hang.kry`. (b) mutually-recursive TWO-struct generics (`A<T> { b: Option<B<Wrap<T>>> }`, `B<T> { a: Option<A<Wrap<T>>> }`, alternating struct names each level) -- tests that `ctx.mono_depth` is a GLOBAL counter, not per-struct-name (a per-name counter would never trip here); fixed binary catches it at depth 301 with the chain correctly alternating `A___i64 -> B___Wrap___i64 -> A___Wrap___Wrap___i64 -> ...`; new file `tests/security/attack_mono_mutual_recursive_structs_growing_no_depth_guard_hang.kry`. (c) growing GENERIC IMPL METHOD (`impl<T> Box<T> { fn wrap(self) -> Box<Wrap<T>> { ... return w.wrap() } }`) -- tested as a CONTROL since `monomorphize_impl_fn` was never in the item 47 gap; measured (not assumed) it does NOT hang (`kryos run` fails in ~0.25s) but hits a DIFFERENT, pre-existing, unrelated error first (a struct-type-propagation ICE-class diagnostic for self-referential impl methods, \`cannot resolve the struct type for field access\`) -- out of scope for item 47 (not a hang), not fixed, documented as a separate finding in the probe file's own header, not silently folded into this item's claim. (d) TYPE ALIAS wrapping (`type Boxed<T> = Node<T>`) -- enumerated but not built as a separate gate file: `kryos-parser`'s `parse_type_alias` does support generics on the alias itself, but a generic alias is pure sugar resolved by the type checker before MIR sees it (confirmed by reading `substitute_type_expr_to_mir`'s `Generic` arm, which only ever dispatches to `monomorphize_struct`/`monomorphize_enum` by the underlying struct/enum name), so this shape reduces to the already-tested struct/enum path rather than exercising new code -- not built as a redundant gate file, reasoning recorded here instead of silently skipped. NON-REGRESSION (the brief's explicit critical requirement -- legitimate deep-but-finite generics must still compile): `tests/conformance/conf_generics.kry`, `conf_generic_ctor.kry`, `conf_generic_enum.kry`, `conf_generic_underscore_name.kry` all PASS unchanged on the fixed binary. The already-filed `attack_mono_struct_doubling_no_depth_guard.kry` (explicitly named in the brief as needing to keep its OLD behavior) re-run on the fixed binary: still completes in 0.76s printing `built depth 10` then the same pre-existing double-free diagnostics (0-59, suppressed after), rc=0 -- byte-for-byte the same documented behavior, confirming the item 47 fix does not touch or change that unrelated bug. `bash compiler/self-host/test_bootstrap.sh`, run ALONE per doctrine: 16/16 PASS, all 16 self-host modules OK -- the sharpest real-world detector for a too-tight cap, unaffected. `bash tools/loop/kryos-loop.sh gates 2`, run detached and polled per the documented ~35-40min runtime: tier 1 GREEN 21/21 (conformance 65/65 incl. `authority_surface`, which took the documented ~9min and was not mistaken for a hang), tier 2 GREEN 7/7, `tier2 GREEN` -- matching the documented baseline exactly, zero drop, 28 PASS / 0 FAIL total, no stray `kryos.exe` process after (`tasklist` checked). GATED: 3 new regression files -- `tests/security/attack_mono_enum_growing_selfref_no_depth_guard_hang.kry`, `tests/security/attack_mono_mutual_recursive_structs_growing_no_depth_guard_hang.kry` (both real hangs pre-fix, now assert the fast `mono_fatal` E0113 exit via ordinary `kryos run` rc/output -- no separate wall-clock-timeout gate script was added since the fix makes both cases exit in well under 1s deterministically) -- plus a documentation-only control probe, `tests/security/probe_mono_impl_method_growing_already_guarded.kry` (not a hang, kept for the record per rule 4, not wired into any gate). The original attacker's file, `tests/security/attack_mono_struct_growing_selfref_no_depth_guard_hang.kry`, already existed as a committed repro from the OPEN entry and now itself doubles as the struct-side regression pin. NOT FIXED / OUT OF SCOPE, disclosed: the struct-type-propagation ICE found via adjacent-shape (c) above (self-referential generic impl method hits a \`cannot resolve the struct type for field access\` internal error) is a real, separate, already-fast-failing bug -- not logged as a new numbered LEDGER item this session (out of this item's assigned scope), left for a future session to triage and decide whether it merits its own entry. |
| **item 49 (item 45 REOPENED): the enum-array-payload leak on container-slot OVERWRITE (`arr[i] = v` / `m[k] = v`) -- FIXED for both container kinds, both backends** | Closed 2026-08-28. REPRODUCED FIRST (rule 1): `git stash` + full `cargo build --release` (rule 2) on the pre-fix HEAD, then measured `tests/mem/enum_array_overwrite_leak.kry` and `tests/mem/enum_map_overwrite_leak.kry` at 3,000,000 iters -- array AOT 371MB, array JIT 371MB, map AOT 463MB, map JIT 371MB (matching the OPEN entry's own 371MB@3M array number closely). ROOT-CAUSED by reading, confirming the OPEN entry's own hypothesis: `release_if_ne_fn` (kryos-mir/src/lower.rs) covers only Str/Array/Map, so the array/map-index-assignment lowering's "read old, store new, release old if pointer-inequal" sequence silently did NOTHING for a Struct/Enum-valued slot -- no generic runtime function can recursively free an arbitrary struct/enum's heap-owned fields (that needs compile-time layout info, which only codegen has), unlike the pointer-inequality-guarded runtime calls (`kryos_array_release_if_ne` etc.) that cover Str/Array/Map. FIX (4 commits' worth of work, several backend-specific wrinkles discovered only by testing, not guessed): (1) added `Instruction::DropIfNe { old, new, ty, retained_by_store, via_map }` to `kryos-mir::ir` -- `old` is deliberately a plain-I64 local (NOT typed `Struct`/`Enum`) holding the raw handle read back via `kryos_array_get`/`kryos_map_get`, so it survives becoming a cross-block value without hitting LLVM's aggregate-vs-pointer representation split for a bare `MirType::Enum`/`Struct` local; `ty` carries the logical type explicitly instead. `new` is populated by RE-READING the slot via the SAME raw getter AFTER the store completes (not the RHS operand directly) because a Struct/Enum RHS's boxed representation is a call-site-specific codegen detail with no stable comparable MIR-level form. Wired into the array/map-index-assignment lowering (`kryos-mir/src/lower.rs`). (2) `drop_unescaped_str_temps`'s whole-window guard treated the new DropIfNe instruction as an unmodeled shape and bailed out via its `_ => return` catch-all, silently DEFEATING item 45's OWN array-field cleanup for the identical statement (both the enum-literal-array construction AND the new DropIfNe live in one MIR window) -- found by tracing a still-leaking isolated repro back to a missing `drop(_12)` for the pre-clone array literal in `--emit-mir` output. Fixed by adding DropIfNe to the guard's allowlist. (3) Cranelift's `kryos_array_set`/`kryos_map_insert(_str)` codegen UNCONDITIONALLY retains a Struct/Enum 3rd argument (a PRE-EXISTING item 44 WAVE 1 compensating fix for a different danger -- a NAMED variable stored into a container and used again), which nothing ever balances for an unnamed/inline enum RHS (Struct/Enum temps are not `drop_unescaped_str_temps` candidates) -- found via a JIT-only residual leak (~29MB/M) after fixes (1)+(2) already flattened AOT. A naive "call the full drop twice" fix caused a LIVE, CAUGHT double-free (`KRYOS-FREE-DIAG`) on the very FIRST slot occupant (the container's own initializer, never routed through kryos_array_set, so never retained) -- corrected to call `kryos_struct_release_shared` ONCE (neutralizes at most one extra unit; a max of one is provable since nothing else retains a Struct/Enum reaching this position) followed by exactly one normal `emit_drop_for_value`, which is now correct for BOTH histories (never-retained and retained-once) since `old` is provably back to sole ownership by the time that runs either way. (4) LLVM boxes a Struct/Enum 3rd argument DIFFERENTLY per callee: kryos_array_set uses `kryos_calloc` (matching `kryos_free`'s expected header) but kryos_map_insert(_str) uses `kryos_arc_alloc_i64` (an ARC header) -- freeing an arc-boxed value with `kryos_free` silently leaks it (bogus size-class, reported not deallocated, not a crash) rather than crashing, found via an AOT-only residual on the MAP shape specifically (JIT unaffected -- Cranelift boxes uniformly via `kryos_calloc` regardless of container). Fixed by routing the `via_map=true` case through `emit_struct_drop`/`emit_enum_drop_payload` (fields only) + `kryos_arc_release` instead of `kryos_free`. PROVEN BOTH WAYS (rule 3) on the FULL, final fix: `git stash` + full release rebuild -- array AOT/JIT 371MB, map AOT 463MB/JIT 371MB at 3M iters (both containers, both backends, matching the pre-fix numbers above); `git stash pop` + full release rebuild -- all four legs flat at 3-4MB, correct output (`acc` matches the closed-form sum in every run, 10 through 3,000,000 iters). RULE 4 (adjacent shapes enumerated: map-insert overwrite, struct-field assignment holding an enum, a str payload instead of `[i64]`, nested enum-in-struct-in-array, both backends -- TESTED: map-insert overwrite (fixed, see (4) above, both backends), struct-field assignment (LEAKS on both backends, NOT fixed -- an LLVM representation split this session's DropIfNe mechanism cannot handle as-is, a fix attempt produced a build failure and was reverted rather than shipped broken, split off as item 51 with its own repro), str-payload enum construction (LEAKS on both backends, but a DIFFERENT, pre-existing, NOT overwrite-specific bug -- a sibling gap to item 45 that reproduces via a plain push-and-drop with no overwrite involved at all, split off as item 52). NOT reached this session: nested enum-in-struct-in-array (would compound items 49's array-index fix, which IS closed, with item 51's struct-field gap, which is NOT -- the struct-field half would still leak; not separately probed, no new information beyond what items 49 and 51 already establish). GATED: new `tests/mem_enum_overwrite_gate.sh` (Windows-only, PowerShell `PeakWorkingSet64` polling matching `mem_enum_array_push_gate.sh`'s own technique; array AND map legs, AOT AND JIT legs, 4 total, all PASS at 3-4MB against a 50MB ceiling), wired into `.github/workflows/ci.yml` right after the item 45 gate step; SKIPs cleanly off-Windows. Two new characterization-only fixtures for the OPEN residuals (not part of the passing gate): `tests/mem/struct_field_enum_overwrite_leak.kry`, `tests/mem/enum_str_push_leak.kry`. GATES: full `bash tools/loop/kryos-loop.sh gates 2`, run detached and polled per the documented ~35-40min runtime -- tier 1 GREEN 21/21 (conformance 65/65, `authority_surface` took its documented ~9min, not mistaken for a hang), tier 2 GREEN 7/7, `tier2 GREEN` -- matching the documented baseline exactly, zero drop. `bash compiler/self-host/test_bootstrap.sh`, run ALONE per this file's own contention note: 16/16 PASS. No stray `kryos.exe` process before gating (checked via `Get-Process`, none found). Files: `compiler/crates/kryos-mir/src/ir.rs`, `display.rs`, `liveness.rs`, `lower.rs`, `optimize/dce.rs`, `optimize/inline.rs`, `optimize/pure.rs`, `compiler/crates/kryos-codegen-cranelift/src/codegen.rs`, `compiler/crates/kryos-codegen-llvm/src/codegen.rs`, `compiler/crates/kryos-codegen-wasm/src/lib.rs` (DropIfNe is a no-op there -- v0.1 has no heap, matching every other ARC instruction), `tests/mem/enum_array_overwrite_leak.kry`, `tests/mem/enum_map_overwrite_leak.kry`, `tests/mem_enum_overwrite_gate.sh`, `.github/workflows/ci.yml`, this file. Honest residuals, NOT fixed, left OPEN with their own repros: item 51 (struct-field-held enum/struct overwrite) and item 52 (a str-payload EnumVariant construction leak, unrelated to overwrite). |
| **45. LEAK, enum-array-push (kryos-mir temp-drop pass), PUSH SHAPE FIXED on both backends; OVERWRITE SHAPE reopened as item 49 and RE-CLOSED for array/map containers (2026-08-28) -- struct-FIELD overwrite and a str-payload sibling gap split off as items 51/52 -- see item 49's own CLOSED row below** | **CORRECTION 2026-08-28: this row's "full fix landed" framing was too strong and the item was REOPENED as item 49, then item 49 itself closed the array/map-index-assignment shapes (see its own row below) while splitting a struct-field-assignment residual and an unrelated str-payload sibling gap into items 51/52.** The fix below genuinely flattens the fresh-construct-push-and-drop shape (independently re-verified: this row's own gate PASSES at 4MB/5M iters on the pushed binary), but constructing the same enum value into an EXISTING container slot (`arr[i] = v` / map insert) still leaks proportionally -- 66MB at 500k and 371MB at 3M iterations, measured on that same post-fix binary. The likely root cause is a different mechanism than this fix touched: `release_if_ne_fn`'s missing Struct/Enum coverage, which item 44's own WAVE 1 addendum flagged and left unmeasured. Read the rest of this row as the account of a PARTIAL fix. Original entry: REPRODUCED FIRST (rule 1): `tests/mem/enum_array_push_leak.kry` (committed 2026-08-19, `Val.ListV([i64])` constructed fresh and pushed into a scratch array every iteration) confirmed leaking on a full `cargo build --release` (rule 2) HEAD, matching the item's own prior AOT number closely (this session measured ~463MB AOT at 5M iters vs the item's ~454MB). ROOT-CAUSED by reading, not guessing: both `kryos-codegen-llvm/src/codegen.rs` (`RValue::EnumVariant` construction, ~line 7803) and `kryos-codegen-cranelift/src/codegen.rs` (~line 6022) UNCONDITIONALLY clone/dup an Array-typed payload field at enum construction (`kryos_array_dup`), mirroring the struct-literal field-cloning fix that predates this session -- but `kryos-mir/src/lower.rs`'s `drop_unescaped_str_temps` pass, which decides whether a fresh array-literal TEMP still owns its own (now-orphaned, pre-dup) buffer and needs a scope-end `Instruction::Drop`, special-cased `RValue::Struct` for exactly this shape (the `S { xs: [..] }` leak, already fixed) but never got a matching `RValue::EnumVariant` arm -- it fell into the pass's generic `other` catch-all, which treats any mention as a possible escape and silently withholds the drop. FIX: added the missing `RValue::EnumVariant` arm (mirrors the existing `RValue::Struct` arm exactly), `compiler/crates/kryos-mir/src/lower.rs`. CORRECTION to the item's original characterization: 'AOT-only, JIT clean' was a MEASUREMENT ARTIFACT, not a real backend difference -- `kryos run` compiles via Cranelift and execs the result as a CHILD PROCESS (`kryos-cli/src/commands/run.rs`, `Command::new(bin).status()`), so the prior session's PowerShell `PeakWorkingSet64` polling of the outer `kryos.exe run` process (which just blocks in `.status()`) read the wrong process and showed near-flat memory regardless of what the child actually did. Polling the child process directly (by its predictable `$TEMP/<stem>.exe` name) shows JIT leaking at the IDENTICAL rate as AOT, since the missing MIR Drop is backend-agnostic (both codegens consume the same MIR) and both codegens dup the array field unconditionally, as shown above. PROVED BOTH WAYS (rule 3, on the FULL fix including the new gate): pre-fix full release build -- AOT 463MB, JIT 462MB peak RSS at 5,000,000 iters (`tests/mem_enum_array_push_gate.sh`, both legs FAIL against a 50MB ceiling, exit 1); reverted, confirmed `git diff` empty; re-applied, full release rebuild -- AOT 4MB, JIT 3MB at the same 5,000,000 iters (both legs PASS, exit 0). 500k-iter checkpoint also flat post-fix (AOT 4.14MB, JIT 4.15MB) vs the item's own probe convention. GATED: new `tests/mem_enum_array_push_gate.sh` (Windows-only, PowerShell `PeakWorkingSet64` polling matching `mem_plateau_check.sh`'s own fallback technique; the JIT leg polls the `kryos run` CHILD process by name, not the parent, per the correction above), wired into `.github/workflows/ci.yml` right after the Memory-plateau gate step; SKIPs cleanly (exit 0) off-Windows rather than false-passing. GATES: `bash tools/loop/kryos-loop.sh gates 2` tier1+tier2 GREEN (65/65 conformance, `no_double_free` PASS, `minilisp` PASS -- the enum-heaviest existing gate, exercising the exact `RValue::EnumVariant` codegen path this fix touches, both backends clean); `bash compiler/self-host/test_bootstrap.sh` 16/16 PASS (run alone, not concurrently with gates, per this file's own machine-contention note). Commit: see `git log` for `compiler/crates/kryos-mir/src/lower.rs`, `tests/mem_enum_array_push_gate.sh`, `.github/workflows/ci.yml`, this file. |
| **Wave: wasm backend short-circuit-in-loop ICE (item 15) + narrow-int truncation re-verification (2026-08-27)** | REPRODUCED FIRST (rule 1): `kryos build tests/known_failures/wasm_shortcircuit_loop_strcat.kry --backend wasm -o x.wasm` -> rc=1, `type mismatch: expected i64 but nothing on stack (at offset 0x70e)`, matching the wave brief exactly. ROOT-CAUSED via `--emit-mir` (the repro's MIR has a genuinely dead, zero-incoming-edge `bb10: drop(_1); return` epilogue block with NO value, appended after the function's real `return`) plus a scratch debug tool (`compiler/crates/kryos-codegen-wasm/examples/dump_wasm.rs`, temporary, deleted before commit) that parsed the emitted (invalid) module with `wasmparser` and printed a running operand-stack-height simulation per instruction -- pinpointed the exact failing instruction as a bare `Return` at offset 0x70e with 0 items on the value stack inside a function whose signature demands 1 (`[I64] -> [I64]`). MECHANISM: the structured control-flow translator (`emit_function_body`) correctly detects this CFG shape (a short-circuit `&&`'s extra branch block feeding into the real if/else, both nested inside a while loop) is beyond what it can express and cleanly falls back to the dispatch relooper (`emit_function_body_relooper`/`emit_relooper`) -- exactly as designed. The relooper emits an `if pc==i {...}` case for EVERY block position in `func.blocks` UNCONDITIONALLY (`for i in 0..n`), unlike the structured path, which only ever visits blocks actually reachable by walking Goto/Branch/Switch edges from the entry block -- so the dead `bb10` epilogue, which has ZERO incoming edges anywhere in the function and can never execute at runtime, still gets a `pc==9` dispatch case emitted. `wasmparser`'s validator checks EVERY code path statically regardless of dynamic reachability, so a bare `return` (no value) inside a non-void function fails type-checking even though the path is provably dead. FIX: `emit_relooper_terminator`'s `Terminator::Return(None)` arm now pushes a placeholder constant of the function's declared return type (`I32Const(0)`/`I64Const(0)`/`F32Const(0.0)`/`F64Const(0.0)`) before emitting `Return`, mirroring the IDENTICAL fallback `emit_function` already uses for a body that falls off the end without an explicit return -- this is not new policy, it is applying an existing, already-accepted pattern to a second code path that needed it. `compiler/crates/kryos-codegen-wasm/src/lib.rs` only. PROVEN BOTH WAYS (rule 3, twice): (1) reverted the fix, full `cargo build --release` (crate links into kryos-cli, not kryos-rt/kryos-stdlib-native so `-p kryos-cli` suffices per rule 2's own scoping), reran the repro -- exact original error reproduced byte-for-byte (`0x70e`); restored, rebuilt, reran -- rc=0, `node tools/wasm-host/run.mjs` output byte-identical to `kryos run` (`YZZZY`). (2) Same revert/restore cycle independently confirmed the fix ALSO closes `examples/showcase/wordscope.kry`'s wasm leg (the real-program blocker this ICE was originally found through) -- byte-identical to native. BONUS FINDING, proven the same way a third time: `tests/harden-probes/probe_23_string_ops.kry` -- `docs/wasm-contract.md`'s own documented "last remaining gap" (complex control flow / irreducible CFG) -- ALSO falls back to the relooper and hits the SAME bug class at a different offset (`0x92c`); reverting reproduces it, restoring fixes it, output byte-identical to native. **Narrow-int truncation (brief's second task):** the plain-local case was already fixed and gated 2026-08-14 (LEDGER's own prior CLOSED entry, `tests/harden-probes/probe_narrow_int_wrap.kry`) -- the brief's premise that a live repro needed to be found was based on a STALE `tests/known_failures/README.md` row describing a file that no longer exists. Re-verified live anyway, fresh (not trusted from the old entry): 6 narrow widths on plain locals, wasm matches both native backends exactly. The STRUCT-FIELD shape (`s.field = s.field + n`) was genuinely never gated against wasm -- `tests/conformance/conf_narrow_struct_field_store.kry` calls `exit()`, which wasm rejects at compile time, so that conformance file silently never ran on wasm at all. Manually verified live across 6 field widths, a two-adjacent-narrow-fields layout, and a narrow-field-next-to-a-wide-field layout (the exact shape that corrupted a neighbor on the historical AOT bug) -- wasm already produces byte-identical output to both native backends in every case, because wasm represents every struct field as its own full i64 array slot (`kryos_array_get`/`kryos_array_set`), so there is no narrower-than-i64 storage for a value to overflow the way LLVM's native struct layout once did. No fix needed for this half; gated going forward (it previously had zero wasm coverage) rather than left as an untested assumption. GATES: `wasm_differential_gate.sh` 65/65 compiled programs agree with native, 0 miscompiles, 0 compile-refusals (was 61/62 with probe 23 refused, before this fix) -- 65/65 after also adding `probe_narrow_struct_field_wrap.kry` -- 2 files added to `tests/harden-probes/` (`probe_wasm_shortcircuit_loop_strcat.kry`, `probe_narrow_struct_field_wrap.kry`). `tests/run_examples_e2e.sh` layer 1b (wordscope's wasm leg) changed from a documented, non-fatal SKIP to a hard `fail=1` on a build regression. Full `bash tools/loop/kryos-loop.sh gates 2`: GREEN, all 27 checks PASS (conformance count read from the gate's own output, not assumed: 65/65). One run mid-session showed `examples_e2e` FAIL under heavy concurrent load from other suites (`task_api[jit]` port-7981 contention, an existing flake unrelated to this wave's changes -- see `tests/run_examples_e2e.sh`'s own SKIP-not-fail handling for that exact case); reran standalone (PASS, rc=0) and reran the full ladder again (tier2 GREEN, all 27 PASS including `examples_e2e`) to confirm it was transient machine contention, not a regression -- not fixed or investigated further since it predates and is unrelated to this wave. `bash compiler/self-host/test_bootstrap.sh`: 16/16 (this wave never touches kryos-rt/kryos-stdlib-native/the self-host compiler surface, run for completeness per rule 6). Regression: `tests/known_failures/wasm_shortcircuit_loop_strcat.kry` deleted per rule 9, folded into `tests/harden-probes/probe_wasm_shortcircuit_loop_strcat.kry`; `tests/known_failures/README.md`'s stale `wasm_narrow_int_no_truncation.kry` row (describing an already-fixed, nonexistent-file bug) removed, and a FIXED entry added for the shortcircuit ICE closure plus the probe-23 bonus. Docs: `docs/wasm-contract.md` differential count 61/62 -> 65/65, the shortcircuit ICE section rewritten from "not fixed, tracked as OPEN" to the fix + root cause, the probe-23 "last remaining gap" section rewritten to CLOSED, the roadmap's irreducible-CFG item struck through as DONE, and a new narrow-int-struct-field paragraph added documenting the re-verification. Scratch debug artifact `compiler/crates/kryos-codegen-wasm/examples/dump_wasm.rs` used to pinpoint the exact failing instruction was deleted before this commit (diagnostic tool, not part of the shipped fix). |
...
| **item 40c: `std::result::to_array<T>` unannotated binding silently rendered a raw pointer, and item 40b's own CLOSED entry falsely claimed otherwise -- PARTIALLY FIXED, REOPENED as item 48** | **CORRECTION 2026-08-28: this row's "FIXED" claim is too strong and the item is REOPENED -- see item 48 in the OPEN section.** The fix below is real for the BARE `let x = call(..)` binding it was built against, but the check never walks destructuring patterns, so `let (a, n) = (to_array(Ok("hi-there")), 5)` still passes `kryos check` clean and still prints a raw pointer on both backends (re-measured live on the pushed post-fix binary, 2026-08-28). Read the rest of this row as the account of a PARTIAL fix. Original entry: Closed 2026-08-27. REPRODUCED FIRST, live, on the current release binary before touching anything: `use std::result::{to_array}  fn main() { let a = to_array(Ok("hi-there"))  println(a[0]) }` printed a raw pointer (`140697...`), `kryos check` rc=0 (clean, no diagnostic), matching the OPEN entry's own numbers exactly. ROOT CAUSE: `to_array<T>(r: Result) -> [T]`'s param is the hand-rolled `std::result::Result` enum (payload `any`), NOT the compiler's builtin generic `Type::Result{ok,err}` -- confirmed by reading `resolve_type_expr`, a bare `Result` name resolves via `env.lookup_enum` to `Type::Enum{name:"Result"}`, never through the `TypeExpr::Generic` `"Result" => Type::Result{..}` arm. So `T` has NOTHING to unify against on the argument side, structurally, for ANY argument -- this is a property of the DECLARED signature, not of any particular call site. FIX SHAPE CHOSEN: (b) from the OPEN entry's own options, adapted -- reject an unannotated binding whose callee's generic type parameter never appears in the callee's own PARAMETER list (a pure signature-shape check on the stored `FunctionSig`, computed once via a new `type_mentions_var` walk, independent of the call site's argument types or any later use in the function). Shape (a) (give `Result` a real typed payload) was correctly ruled out as too large/cascading for this session, matching the OPEN entry's own risk assessment. FIRST ATTEMPT REVERTED, not shipped: a deferred "record every unannotated generic-call let, then at END-OF-MODULE check whether the var resolved to concrete" design (mirroring the existing empty-array/empty-map `resolved_let_types` mechanism) was built, tested green on `tests/type_soundness.sh`, then caught by running the FULL conformance suite before committing: `tests/conformance/conf_generics.kry` regressed with 5 bogus `E0110`s at WRONG spans (pointing at unrelated struct-field/comment lines), because `push<T>(arr: [T], val: T) -> [T]` -- a normal, correctly-bindable generic builtin -- was flagged every time it was called from inside another generic struct/method's OWN body: the checker's single pass over a generic template only ever sees the ENCLOSING generic's abstract placeholder var, which never looks "resolved to concrete" during that one pass even though every real monomorphized call site is sound. Reverted the deferred design entirely (field, recording code, end-of-module check) in favor of the static signature-shape check above, which does not depend on resolution state at all and cannot see that class of false positive. PROOF BOTH WAYS, live, TWICE (once per design iteration): `git stash` the check.rs fix alone, full `cargo build --release`, rerun -- the item 40c repro's `check` passes clean (rc=0) and prints the raw pointer again (reproduced fresh, not cached), and the 3 new `tests/type_soundness.sh` probes go RED (`HOLE to_array_unannotated_binding_silent_wrong_answer`, `..._err_case`, `..._not_rescued_by_later_use` -- "unsound program passed kryos check"); `git stash pop`, rebuild, rerun -- GREEN (`type-soundness: all probes correct`). PROBED THE CLASS per the wave's instructions: `Err(...)` case also correctly rejected (same mechanism, T unbound regardless of which Result variant); `std::option::to_array<T>(opt: Option<T>) -> [T]` is NOT affected -- confirmed live, unannotated `to_array(some("hi-opt"))` -> `hi-opt` correctly, because `Option<T>` in that signature IS the real generic type (its own `T` appears in the PARAM), unlike `Result`; `std::iter::count<T>(arr: [T]) -> i64` likewise unaffected (`T` appears in the param) -- live-verified unchanged; nested-in-a-function-argument (`first(to_array(Ok("nested-hi")))` with `first(xs: [str])`) was ALREADY fine before this fix and remains fine -- that shape never goes through an unannotated LET at all, so it was never item 40c's bug class; both backends confirmed -- `kryos run` AND `kryos build --release` on the repro both now exit rc=1 with the same `E0110`, where both previously exited rc=0 and printed the wrong value. NEW REGRESSION TEST added to `tests/type_soundness.sh` (a real gate, not `known_failures/` -- this item had no `known_failures/` repro file to retire): `to_array_unannotated_binding_silent_wrong_answer`, `to_array_unannotated_binding_err_case`, and `to_array_unannotated_binding_not_rescued_by_later_use` (`want_reject`, documents the deliberate design tradeoff -- no deferred "later use rescues it" leniency, on purpose, per the reverted-attempt finding above), plus a NO-CASCADE complement `push_generic_unannotated_binding_still_works` (`want_pass`) pinning the exact shape that regressed on the first attempt so it can never silently regress again. DOC updated: `docs/stdlib/result.md`'s `to_array` entry corrected from "silent wrong answer" to "clean `E0110` compile error", with the annotation workaround kept. EVIDENCE, all fresh this session, `KRYOS_STDLIB_DIR` + release binary: full `cargo build --release` clean; `tests/type_soundness.sh` all probes correct; `tools/loop/kryos-loop.sh gates 2` -- tier 1 GREEN (conformance 65/65 PASS + 20 more gates PASS) and tier 2 GREEN (7/7 gates PASS, `tier2 GREEN`), including `conformance` 65/65 (the exact suite the reverted first attempt broke) and `capability_matrix`/`strict_caps`/`examples`/`examples_e2e`/`ir_signatures`/`selfhost_wholeprogram` all clean; `compiler/self-host/test_bootstrap.sh` run ALONE per doctrine -- **16/16 PASS**, all 16 self-host modules OK. NOT FIXED / OUT OF SCOPE, disclosed: shape (a) (a real typed `Result<T,E>` payload in `std::result`) remains the deeper fix and is not attempted; any OTHER hand-rolled stdlib generic whose own type param is return-only and never appears in its params would hit the same static check and require annotation too -- none found in a repo-wide grep of `compiler/stdlib/*.kry` for `fn .*<.*>.*Result)` beyond `to_array` itself, but the check is general, not `to_array`-specific, so a future such signature is caught automatically rather than needing a new point-fix. **2026-08-28 addendum: item 48 (this row's destructuring-pattern residual) is now CLOSED -- see the item 48 row below.** |
| **item 48: item 40c's unbindable-generic check only ever matched a BARE `let x = generic_fn(..)` binding -- a destructuring `let (a, n) = (to_array(Ok("hi-there")), 5)` bypassed it entirely, passing `kryos check` clean (rc=0) and printing a raw pointer at runtime on both backends -- FIXED** | Closed 2026-08-28. REPRODUCED FIRST (rule 1), live, on the pushed post-40c release binary before touching anything: `use std::result::{to_array}  fn main() { let (a, n) = (to_array(Ok("hi-there")), 5)  println(a[0]) }` -> `kryos check` rc=0 (clean, no diagnostic), `kryos run` printed `140701993349120` (a raw pointer), matching the OPEN entry's own numbers exactly. ROOT CAUSE (read, not guessed): the item 40c check (`compiler/crates/kryos-types/src/check.rs`, `Stmt::Let` handling) only fired inside `if let Some(Expr::FnCall {..}) = value.as_ref()` -- for a destructuring `let`, `value` is a `TupleLiteral` (or `ArrayLiteral`/`StructLiteral`), never a bare `FnCall`, so the `if let` never matched regardless of what the pattern destructured. FIX: replaced that single-shape check with `check_unbindable_generic_binding(pattern, value)`, a new recursive walker (plus its extracted leaf helper `report_unbindable_generic_call`) that walks the PATTERN and VALUE in lockstep: `Pattern::Tuple` against `Expr::TupleLiteral` (positional, any nesting depth), `Pattern::Struct` against `Expr::StructLiteral` (by field name), and -- for a non-destructured single-name binding -- recurses into `Expr::ArrayLiteral`/`Expr::TupleLiteral` elements too, since neither has a per-slot DECLARED type to constrain a generic return against (unlike a struct literal field, which is soundly constrained by the struct's own declared field type -- proved correct at RUNTIME by a dedicated no-cascade probe, not just "not rejected"). `compiler/crates/kryos-types/src/check.rs` only. RULE 4 (enumerate the shapes not thought of): tested tuple-destructure first element, non-first element, nested tuple, the `Err(..)` case, a struct pattern NESTED inside a tuple pattern (a top-level `let Name {..} = ..` does not parse as a struct pattern at all -- `parse_let` only recognizes a leading `(` for destructuring, confirmed by reading the parser -- so this is the only reachable struct-pattern shape), an array literal wrapping the call with no pattern at all, a bare tuple bound to a single name with no pattern, the call inside a closure body, and the call passed directly as a call argument. REPRODUCED AND GATED (all `want_reject`): tuple-first, tuple-nonfirst, nested-tuple, Err-case, bare-tuple-no-pattern, array-literal, and -- found live during enumeration, NOT in the original brief -- a struct pattern nested inside a tuple pattern, which ground-truth-probed (fix arm temporarily disabled, rebuilt, rerun) to a REAL, reachable runtime PANIC ("array is null"), not just a silent wrong value: `bind_pattern_with_mut`'s own `Pattern::Struct` arm binds each field through a brand-new `fresh_var()` never linked back to the struct's actual declared field type, so it has no external constraint to fall back on the way the (sound) non-destructured struct-literal-field shape does. Did NOT reproduce (confirmed by running, not assumed): a plain (non-destructured) `let h = Holder { a: to_array(..), .. }` -- prints the correct string, `T` is soundly bound by the struct's declared field type during ordinary field unification, same mechanism as -- and confirmed alongside -- a generic call passed directly as an argument to a function with a concrete param type (`show(to_array(Ok("hi-there")))` also prints correctly, unchanged). Closure-body `let a = to_array(..)` was ALREADY covered by the original bare-let check (not a new shape). `array-patterns` (named in the brief `by the same argument`) do not exist in Kryos at all -- confirmed by reading `kryos-ast`'s `Pattern` enum (`Wildcard`/`Ident`/`Literal`/`Tuple`/`Struct`/`Enum`/`Or`, no `Array` variant) -- so that half of the brief's fix shape is inapplicable, not skipped. PROVEN BOTH WAYS (rule 3), TWICE: (1) the manual repro -- `git stash` the check.rs fix alone, full `cargo build --release`, reran -- `check` rc=0 clean and `run` printed a fresh raw pointer (`140700479074304`, confirmed not cached); `git stash pop` (diffed byte-identical to the pre-stash file), full rebuild, reran -- `E0110` at the exact call span, `run` also rejects (rc=1). (2) the gate itself -- same stash/rebuild cycle with the 11 new `tests/type_soundness.sh` probes already added: all 7 `want_reject` probes went `HOLE ... unsound program passed kryos check` red without the fix; restored + rebuilt -- `type-soundness: all probes correct`. GATED: `tests/type_soundness.sh` gained 7 `want_reject` probes (`to_array_unannotated_tuple_destructure_first_element`, `..._nonfirst_element`, `..._err_case`, `..._nested_tuple_destructure`, `..._struct_pattern_nested_in_tuple`, `..._inside_array_literal`, `..._bare_tuple_no_pattern`) and 4 `want_pass` no-cascade complements (`to_array_in_struct_literal_field_still_works`, `to_array_as_call_argument_still_works` -- both proven correct at runtime, not just accepted; `push_generic_still_works_inside_tuple_destructure` -- guards the Tuple-walk against the exact false-positive class item 40c's first reverted attempt hit for `push` inside a generic struct method; `normal_call_still_works_inside_array_literal` -- guards the new Array-literal walk against over-triggering on ordinary non-generic code). EVIDENCE, all fresh this session, `KRYOS_STDLIB_DIR` + release binary: full `cargo build --release` clean (no `-p`, per rule 2); `tests/type_soundness.sh` all probes correct (was run both red-without-fix and green-with-fix, see PROVEN BOTH WAYS above); `tools/loop/kryos-loop.sh gates 2` run detached per the documented ~35-40min runtime -- tier 1 GREEN 21/21 (conformance 65/65) and tier 2 GREEN 7/7, `tier2 GREEN`, matching the documented baseline exactly, no drop; `compiler/self-host/test_bootstrap.sh` run ALONE per doctrine -- 16/16 PASS, all 16 self-host modules OK. NOT FIXED / OUT OF SCOPE, disclosed: the qualified-path-generic-call link-failure finding noted in the original OPEN entry was explicitly a SEPARATE bug class (a build failure, not a silent wrong answer) and was never part of this item's scope -- split off to its own tracked item 50 (OPEN section) rather than silently dropped when this item's heading was removed. `if let`/`while let` destructuring an unbindable-generic call (these desugar to `match`, not `Stmt::Let`, so this fix's `Stmt::Let`-specific check does not cover them) was not tested and is not known to reproduce or not -- flagged as unexplored, not silently assumed safe. |
