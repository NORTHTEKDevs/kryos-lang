...
### 40b. `str` routed through an `[any]` container renders a raw POINTER instead of the string -- documented limitation, DELIBERATELY NOT BLOCKED (split out of item 40 when it was fixed, 2026-08-14)

Measured 2026-08-14 while building item 40's no-cascade complement:

```
fn log_event(args: [any]) { println(to_string(args[0])) }
fn main() { log_event(["hello"]) }
      -> logged: 140698792906752      (a pointer, not "hello")
```

This is a DIFFERENT mechanism from item 40's bool/f64 case and is why item
40's fix does not cover it. `bool`/`f64` are REINTERPRETED -- the value is
genuinely corrupted and the two backends disagree (`true` -> JIT `1`, AOT
`-1`), so rejecting is the only honest answer. A `str` is a POINTER, which
is already i64-shaped; the handle survives intact and only the RENDERING is
wrong. That is the long-standing item 6 / CLAUDE.md gotcha #22 erasure.

NOT BLOCKED, deliberately, and the reason is measured rather than assumed:
`[any]` still appears in live stdlib signatures (`std::test::run_tests`,
`std::result::to_array`, `std::iter::count`), and `std::fmt::format` was
ALREADY migrated `[any]` -> `[str]` in an earlier session for exactly this
reason (see `tests/conformance/conf_stdlib_correctness_sweep.kry`'s header).
Rejecting `str` at `[any]` boundaries would cascade into those remaining
signatures for a rendering bug whose value is not corrupted. The migration
away from `[any]` is the real fix and is already underway.

`tests/type_soundness.sh` deliberately does NOT assert that `str` through
`[any]` works -- an earlier draft of item 40's no-cascade probe claimed
exactly that and was wrong; it now pins only `i64`, with this limitation
named in a comment so the next reader does not re-derive it.
