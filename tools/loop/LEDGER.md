...
### 52. LEAK -- sibling gap to item 45, NOT overwrite-specific: an Enum construction with a STR payload field (`Enum.V(str_expr)`) leaks the pre-clone string on every construction, push OR overwrite (found while testing item 49's adjacent shapes, 2026-08-28) -- NOT FIXED, newly logged

`drop_unescaped_str_temps` (`kryos-mir/src/lower.rs`) got an
`RValue::EnumVariant`-with-Array-field arm from item 45's fix, but never a
matching Str-field arm. An `EnumVariant` construction CLONES a `str`
payload field (`kryos_string_clone`, confirmed in BOTH backends' codegen)
exactly like it clones/dups an Array-typed field -- so the pre-clone string
temp is orphaned on every construction, leaking one buffer per
Enum-with-str-payload construction regardless of whether the enum is later
pushed, overwritten, or just dropped. This is UNRELATED to container-slot
overwrite specifically and predates/is unaffected by item 49's fix either
way -- reproduces via item 45's OWN push-and-drop shape with a `str`
payload substituted for the `[i64]` one, no overwrite involved at all.
Measured: ~78MB/M on AOT, ~78MB/M on JIT (shared MIR root cause, confirmed
by both backends leaking at the same rate).
Reproduce: `tests/mem/enum_str_push_leak.kry`.
