...
**Exception: a captured CLOSURE or fn-value does NOT snapshot -- it is shared, and by design.** Every other capture kind above (str/array/map/struct/enum) is deep-copied so the spawned thread privately owns it. A closure or bare function value captured into a `spawn` block is instead reference-counted and SHARED (`kryos_arc_retain`, not a copy) -- if the same closure value is captured by more than one `spawn` (or by `spawn` and also called from the parent thread), every caller shares the exact same closure environment. This is intentional: it is what lets a single stateful closure (e.g. a shared counter/accumulator idiom, `let bump = || { n = n + 1  n }`) act as one logical piece of shared state across threads, the same way a `Mutex`/`atomic_int`/actor does.
