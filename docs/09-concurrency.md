...
Kryos's concurrency model is `spawn` for fire-and-forget OS-thread parallelism, `chan<T>` channels (MPMC, blocking `recv` / non-blocking `try_recv`) with `select`, a cooperative `async`/`await` executor (`coop_spawn` / `coop_run`) that genuinely interleaves tasks, and `actor` for stateful message-passing (each actor owns private state and processes messages one at a time on its own thread). All are built into the language syntax -- no library imports, no callback chains.
