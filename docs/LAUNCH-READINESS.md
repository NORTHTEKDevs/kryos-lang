...
> it was `VERSIONING.md`'s own bar: *"cut only after external users have run
> real workloads against it."* **That bar is still unmet. Kryos still has one
> user.** The project owner waived the precondition on 2026-08-29 and 1.0.0 was
> tagged. The waiver, and exactly what it does and does not claim, is recorded
> in [`VERSIONING.md`](../VERSIONING.md); it is not deleted there and §1 is not
> rewritten here.
>
> What that means for a reader of this document:
>
> - **§1's engineering findings still stand and were re-measured at the cut.**
>   Zero known capability escapes, full gate ladder green, byte-identical
>   bootstrap 16/16, ecosystem 259/259, conformance 68/68 on both backends.
> - **§1's exposure finding also still stands.** No stranger has stress-tested
>   this compiler. Every number in this repository comes from its own gates on
>   its own machine.
> - **New since §1, and disclosed rather than absorbed:** four defects were
>   found and fixed between 2026-08-28 and 2026-08-29 (LEDGER items 50, 52, 53,
>   54), two of which were *silent-wrong-answer or corruption* class and two of
>   whose first fixes were themselves wrong in the corruption direction, caught
>   by the gate ladder. That is evidence adversarial search was still finding
>   real defects at the moment of the cut, which is exactly the argument §1 made
>   for wanting external exposure. It is recorded here so the cut is not read as
>   "nothing was left to find."
> - **Known limitations ship documented**, with measured costs, in
>   [`STABILITY.md` §5](../STABILITY.md) - LEDGER items 3, 51 and 41.
>
> Treat v1.0.0 as a **stability commitment** (the CLI/LSP/stdlib/ABI surface is
> frozen under semver) and **not** as evidence of field-hardening. The project
> renumbered `v1.0.0-rc.2` down to `v0.9.0` on 2026-08-20 to avoid conflating
> those two claims, and this addendum exists so the 1.0.0 cut does not quietly
> re-conflate them.
